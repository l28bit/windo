...
- **`windo repair`** / **`windo repair keybindings`** — one-step **keybindings safe-reset** (Alt+w, clears legacy WINDO PSReadLine chords) plus hints to reload **`$PROFILE`** and run **`windo install-latest`** when upgrading from older builds.
- **Installer checksum verification** — **`bootstrap.ps1`** and **`windo install-latest`** parse the published [`checksums/installer.sha256`](checksums/installer.sha256) robustly (first 64 hex chars), matching **`Invoke-WebRequest`** body formats (BOM, extra text). Reduces false **SHA256 mismatch** failures.
...
| `windo recipes [list] \| show \| run` / `windo run --recipe <name>` | **v3.2.0+** Built-in elevated **recipe** templates (bundled data, not arbitrary script). **v3.2.6+** includes read-only **Ollama** peeks (**`ollama-list`**, **`ollama-ps`**, **`ollama-version`**) when **`ollama.exe`** is on PATH. |
