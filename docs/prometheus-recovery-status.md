...
The Prometheus validation workflow is installed on both the `main` release target and the current `Exodus` default branch so every synchronization of this recovery branch is evaluated against the same Windows release gate.
