...
- **Search:** `windo extras search [query]` downloads the index from the **`jonex/windo-production-ready`** branch unless you set **`WINDO_EXTRAS_INDEX_URL`** (also shown in **`windo config`** / **`windo config --json`** as **`extrasIndexUrl`**).
