...
Assert-Equal ($installerSource.Contains('$WindoVersion = "4.2.0"') -eq $true) $true "installer version is 4.2.0"
Assert-Equal ($bootstrapSource.Contains("WINDO 4.2.0 Special Edition bootstrap") -eq $true) $true "bootstrap banner is current"
...
Assert-Equal ($bootstrapSource.Contains("contents/windo_install.ps1?ref=Genesis") -eq $true) $true "bootstrap knows GitHub Contents API installer URL"
Assert-Equal ($bootstrapSource.Contains('$Repo = "https://raw.githubusercontent.com/l28bit/windo/Genesis/windo_install.ps1"') -eq $false) $true "bootstrap no longer hardcodes raw installer as primary source"
...
Assert-Equal ($installerSource.Contains("api.github.com/repos/l28bit/windo/contents/checksums/installer.sha256?ref=Genesis") -eq $true) $true "installer uses GitHub Contents API for checksum lookup"
...
Assert-Equal ((Test-Path (Join-Path $Root "docs\releases\RELEASE_NOTES_v4.2.0.md")) -eq $true) $true "v4.2.0 release notes exist"
Assert-Equal ((Test-Path (Join-Path $Root "docs\v5-roadmap.md")) -eq $true) $true "v5 roadmap doc exists"
Assert-Equal ($readmeSource.Contains("RELEASE_NOTES_v4.2.0.md") -eq $true) $true "README links v4.2.0 release notes"
