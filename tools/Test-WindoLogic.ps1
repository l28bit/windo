...
if ($bootstrapBoolFn -and $bootstrapSpinnerFn -and $bootstrapReleaseMetadataFn -and $bootstrapReleaseMetadataStateFn -and $bootstrapParsedPayloadFn -and $bootstrapManifestValueFn -and $bootstrapNormalizedPublishedChecksumFn -and $bootstrapReleaseBranchFn -and $bootstrapErrorResponseFn -and $bootstrapRetryableWebErrorFn -and $bootstrapEnvValueFn -and $bootstrapHasTextFn -and $bootstrapWebRequestFn -and $bootstrapRestMethodFn -and $bootstrapStartProcessFn) {
    ...
    Assert-Equal $false $true "bootstrap exposes bool, spinner, release metadata, strict-safe env, web request, and process launch helpers"
}
...
Assert-Equal ($installerSource.Contains('version = "8.5.9"') -and $installerSource.Contains('Plate Banner')) $true "roadmap includes V8.5.9 plate banner entry"
...
Assert-Equal ($installerSource.Contains('function Write-WindoInstallerPlateLine') -eq $true) $true "installer uses static plate banner lines"
Assert-Equal ($installerSource.Contains('function Write-WindoInstallerPulse') -eq $false) $true "legacy inline pulse banner removed"
Assert-Equal ($installerSource.Contains('function Write-WindoInstallerBootLine') -eq $false) $true "legacy char/token boot line removed"
Assert-Equal ($installerSource.Contains('windo do -- [command]') -eq $true) $true "banner example uses bracket placeholder instead of carrot syntax"
Assert-Equal ($installerSource.Contains('function Test-WindoInstallerBannerEnabled') -eq $true) $true "installer supports quiet plate banner via WINDO_INSTALLER_VISUALS"
