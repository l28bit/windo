...
Assert-Equal ($installerSource.Contains('$WindoVersion = "4.1.0"') -eq $true) $true "installer version is 4.1.0"
Assert-Equal ($bootstrapSource.Contains("WINDO 4.1.0 Special Edition bootstrap") -eq $true) $true "bootstrap banner is current"
...
Assert-Equal ((Test-Path (Join-Path $Root "docs\releases\RELEASE_NOTES_v4.1.0.md")) -eq $true) $true "v4.1.0 release notes exist"
Assert-Equal ((Test-Path (Join-Path $Root "docs\v5-roadmap.md")) -eq $true) $true "v5 roadmap doc exists"
Assert-Equal ($readmeSource.Contains("RELEASE_NOTES_v4.1.0.md") -eq $true) $true "README links v4.1.0 release notes"
