...
- **Native tray launchpad:** **`windo launchpad --tray`** writes a local tray-agent script under **`.pwsh_secure`**, starts it hidden with Windows Forms, shows a shield tray icon and balloon notification, and exposes menu/window actions that open visible PowerShell command windows.
