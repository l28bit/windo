...
- **`windo dashboard`** — operator health view combining task presence, integrity, audit-chain verification, audit categories, and recent entries. Supports terminal output, **`--json`**, **`--html`**, and **`--open`**.
- **Report visuals** — **`windo report`** now uses summary cards and category bars in the local HTML report.
- **Checksum guardrails** — local validation now checks the normalized published installer checksum so release edits fail fast when [`checksums/installer.sha256`](checksums/installer.sha256) is stale.
