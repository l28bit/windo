...
Use `windo roadmap` or `windo roadmap --json` to inspect this release train from the installed profile. Use `windo mesh workbench`, `windo surface`, `windo motion`, `windo trust`, `windo syntax doctor`, `windo explain`, and `windo source` to inspect the shipped foundation.
