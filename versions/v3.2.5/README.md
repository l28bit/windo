...
- **AI / agent CLI bridge (hygiene)** — [`docs/ai-bridge.md`](docs/ai-bridge.md) describes safe patterns for OpenAI and other tools; **`windo ai status`** and **`windo ai doctor`** list which common API-key **names** are set in Process/User/Machine scope (**values never shown**). WINDO does not call vendor APIs or store keys—see [`SECURITY.md`](SECURITY.md).
...
**Optional shell companion layer** (modules, recipes, prompt bridge, curated extras—**v3.2.0+**): see [`docs/framework-wave.md`](docs/framework-wave.md) for how these features map to the shipped plan and trust model. For **AI/agent CLIs** and API-key discipline, see [`docs/ai-bridge.md`](docs/ai-bridge.md).
