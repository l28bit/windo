<# =====================================================================
WINDO v3.2.5 Release-Hardened Installer
...
$WindoBuiltinVerbs = @(
    ...
    'modules', 'recipes', 'prompt', 'extras', 'dev', 'session', 'ai', 'run'
)
...
function windo {
    ...
    function _windo_ai_credential_env_snapshot {
        ...
        $anyProc = $false; foreach ($k in @($proc.Keys)) { if ($proc[$k]) { $anyProc = $true; break } }
        $anyUser = $false; foreach ($k in @($user.Keys)) { if ($user[$k]) { $anyUser = $true; break } }
        $anyMach = $false; foreach ($k in @($machine.Keys)) { if ($machine[$k]) { $anyMach = $true; break } }
        @{
            elevated              = (_windo_is_process_elevated)
            processHasSecret      = $anyProc
            userScopeHasSecret    = $anyUser
            machineScopeHasSecret = $anyMach
    ...
    function _windo_help_topics {
        return @(
            ...
            [pscustomobject]@{
                ...
                Examples    = @("windo recipes", "windo recipes show firewall-profiles", "windo recipes run ipconfig-brief", "windo run --recipe os-version")
            },
            [pscustomobject]@{
                Name        = "prompt"
                Category    = "Shell Experience"
            ...
            [pscustomobject]@{
                Name        = "ai"
                Category    = "Shell Experience"
                Summary     = "AI / agent CLI hygiene: credential env visibility (names only; never prints secrets)."
                Syntax      = @("windo ai [status] [--json]", "windo ai doctor [--json]")
                Description = "Read-only snapshot of common API-key-related environment variables across Process, User, and Machine scopes. Use to verify you are not carrying secrets into elevated sessions or system-wide env. Does not call remote APIs or store keys."
...
"@
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "ai") {
        ...
        if ($snap.machineScopeHasSecret) {
            [void]$issues.Add("At least one AI-related variable is set at Machine (system-wide) scope; prefer per-user or vault-backed storage.")
        }
        ...
        if ($sub -eq "doctor") {
            ...
            [void]$rec.Add("See docs/ai-bridge.md for OpenAI CLI, agents, and IDE integration patterns.")
        }
        $exitPayload = 0
        ...
        if ($JsonOutput) { _emit_json "ai" $pl } else {
            Write-Host "[windo] ai $sub — API key env visibility (names only; values never shown)" -ForegroundColor Cyan
