<# =====================================================================
WINDO v5.1.0 Limited Edition Installer
...
$WindoBuiltinVerbs = @(
    ...
    'theme', 'output', 'motion', 'surface', 'control', 'signal', 'center', 'edition', 'upgrade', 'install-latest', 'uninstall', 'remove', 'profile', 'keybindings', 'completion', 'roadmap', 'syntax', 'mesh',
...
"@
    ...
    function _windo_edition_state {
        ...
        [pscustomobject]@{
            ...
            commands = @("windo edition open", "windo center open", "windo center actions", "windo signal open", "windo control preview surface-prime")
...
"@
    ...
    function _windo_help_topics {
        return @(
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo center [status] [--json]", "windo center open|tray", "windo center actions|signal|history", "windo center preview|run|queue <action-id>", "windo center execute-next|next", "windo center execute <request-id>")
                ...
                Examples    = @("windo center", "windo center actions", "windo center preview surface-prime", "windo center queue surface-prime", "windo center execute-next", "windo center signal")
            },
            [pscustomobject]@{
                Name        = "edition"
                Category    = "Shell Experience"
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "center") {
        ...
        if ($sub -notin @("status", "")) {
            if ($JsonOutput) { _emit_json "center" @{ error = "expected status | open | tray | actions | preview | run | queue | execute-next | next | execute | history | signal"; exitCode = 2 } } else { Write-Host "[windo] center: expected status | open | tray | actions | preview | run | queue | execute-next | next | execute | history | signal" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        $center = _windo_center_state
...
function __windo_completion_specs {
    @{
        help = @('version','install-latest','source','trust','scan','vault','sshx','crypto','explain','syntax','mesh','completion','output','motion','surface','control','signal','center','edition','keybindings','recipes','venv','pkg','launchpad','preflight','dashboard','integrity','verify','config','profile','modules','extras','ai','repair')
        ...
        control = @('status','prime','actions','preview','queue','run','execute-next','next','execute','inspect','cancel','history','pulse','demo','clear','surface-status','surface-prime','launchpad-tray','workbench-html','edition-open','motion-pulse','profile-doctor','trust-online','preflight','install-latest','--json')
        signal = @('status','timeline','last','export','open','html','--html','--open','--output','--json')
        center = @('status','open','tray','actions','preview','run','queue','execute-next','next','execute','history','signal','surface-status','surface-prime','launchpad-tray','workbench-html','edition-open','motion-pulse','profile-doctor','trust-online','preflight','install-latest','--json')
