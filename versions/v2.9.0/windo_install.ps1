<# =====================================================================
WINDO v2.9.0 Release-Hardened Installer
...
function windo {
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "help") {
        $helpText = @(
            ...
            "windo stats [--since YYYY-MM-DD] [--last-days N]   Summarize audit log (filters by entry Timestamp)",
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "stats") {
        ...
        while ($sxi -lt $Command.Count) {
            ...
            if ($a -eq "--since" -and $sxi + 1 -lt $Command.Count) {
                $sinceStr = [string]$Command[$sxi + 1]
                $sxi += 2
                continue
            }
            if ($a -like "--since=*") {
                $sinceStr = $a.Substring(8)
                $sxi++
                continue
            }
            if ($a -eq "--last-days" -and $sxi + 1 -lt $Command.Count) {
                try { $lastDaysN = [int]$Command[$sxi + 1] } catch { $lastDaysN = $null }
                $sxi += 2
                continue
            }
            if ($a -like "--last-days=*") {
                try { $lastDaysN = [int]($a -replace '^--last-days=', '') } catch { $lastDaysN = $null }
                $sxi++
                continue
            }
            $sxi++
        ...
        if ($null -ne $sinceDt -and $null -ne $lastDaysN) {
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "doctor") {
        ...
        Write-Host "    WINDO_MAX_COMMAND_CHARS, WINDO_SKIP_INSTALLER_SHA256  (see README / SECURITY)" -ForegroundColor DarkGray
