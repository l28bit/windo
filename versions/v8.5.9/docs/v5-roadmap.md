...
| 8.5.6 | Dr. Run | Fortify task runner download, repair, config, and cleanup workflows. | Shipped |
| 8.5.7 | Midflight Fuel | Curated repair lanes for every non-OK preflight finding. | Current |
...
### 8.5.7 Midflight Fuel

- Installer/bootstrap branding: `WINDO 8.5.7 V8.5`.
- `windo preflight` includes `repairCommand` and `repairPlan` when checks are not OK.
- `windo midflightfuel` applies curated repair lanes only; no arbitrary command execution.
- Broken-profile recovery keeps a bootstrap rescue command visible for updates from very old WINDO versions.
