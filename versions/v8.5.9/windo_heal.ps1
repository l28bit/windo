...
$WindoHome = [string]$env:USERPROFILE
if ([string]::IsNullOrWhiteSpace($WindoHome)) {
    throw "USERPROFILE is required to run WINDO Heal."
}
$SecureDir = Join-Path $WindoHome ".pwsh_secure"
$RuntimePath = Join-Path $SecureDir "windo_runtime.ps1"
$SnapshotDir = Join-Path (Join-Path $WindoHome "Documents") "windo"
...
function Get-WindoProfilePaths {
    ...
    $paths += @(
        (Join-Path $WindoHome 'Documents\PowerShell\Microsoft.PowerShell_profile.ps1'),
        (Join-Path $WindoHome 'Documents\WindowsPowerShell\Microsoft.PowerShell_profile.ps1'),
        (Join-Path $WindoHome 'Documents\WindowsPowerShell\Profile.ps1')
    )
...
# WINDO-CUSTOM-PROFILE-D: $(Join-Path (Join-Path $WindoHome 'Documents') 'windo\profile.d')
...
Write-Host "  [Net.ServicePointManager]::SecurityProtocol = [Net.ServicePointManager]::SecurityProtocol -bor [Net.SecurityProtocolType]::Tls12; iex (irm 'https://raw.githubusercontent.com/l28bit/windo/jonex/windo-production-ready/bootstrap.ps1')" -ForegroundColor DarkGray
