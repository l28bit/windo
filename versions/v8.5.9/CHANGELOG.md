...
See README "What's new in WINDO V8.5.9".

## [8.5.7] - 2026-05-29

Release notes: [`docs/releases/RELEASE_NOTES_v8.5.7.md`](docs/releases/RELEASE_NOTES_v8.5.7.md).

### Added

- `windo midflightfuel` applies curated repair lanes for non-OK `windo preflight` findings, with `--dry-run`, `--only`, `--force`, and `--json`.
- Preflight JSON now includes `repairCommand` and a `repairPlan` so automation can identify red/yellow checks and the supported repair route.

### Changed

- `windo preflight` now ends with the explicit repair option `windo midflightfuel` when any check is red or yellow.
- Preflight check rows now carry `repairAction` and `repairCommand` fields in addition to the human `fixCommand`.

### Reliability

- Legacy/broken-profile recovery guidance now points to the current Exodus bootstrap rescue path when the installed `windo` function is too old or damaged to update itself.

## [8.5.6] - 2026-05-29

Release notes: [`docs/releases/RELEASE_NOTES_v8.5.6.md`](docs/releases/RELEASE_NOTES_v8.5.6.md).

### Added

- `windo runner doctor`, `status`, `config`, `cleanup`, and `repair` for Dr. Run task-runner lifecycle inspection, config visibility, explicit repair planning, and stale artifact cleanup.

### Changed

- `windo doctor --json` and `windo config --json` now include runner lifecycle/config posture; text output shows compact Dr. Run state.
- `windo cleanup` no longer blindly removes runner request/result JSON. Runner artifact cleanup is dry-run unless explicitly applied.
- Installer manifest now records a schema version and runner lifecycle command references for repair/config/cleanup discovery.

### Security

- Runner cleanup rejects prefix-collision paths and only targets known WINDO request/result/temp patterns under `.pwsh_secure`.

## [8.5.5] - 2026-05-26

Release notes: [`docs/releases/RELEASE_NOTES_v8.5.5.md`](docs/releases/RELEASE_NOTES_v8.5.5.md).

### Fixed

- Bootstrap elevated installer handoff no longer defines a wrapper parameter named `ErrorAction`, avoiding common-parameter collision failures before UAC launch completes.

### Tests

- Added bootstrap launch-wrapper regression coverage for the `Start-Process` handoff contract.

## [8.5.4] - 2026-05-25

Release notes: [`docs/releases/RELEASE_NOTES_v8.5.4.md`](docs/releases/RELEASE_NOTES_v8.5.4.md).

### Fixed

- Generated profile command helpers now load before argument parsing uses them, fixing `_windo_set_exit` not recognized failures across `install-latest`, `upgrade`, `integrity`, and `doctor`.
- Timeout flag parsing now has the same prelude ordering protection.

### Tests

- Added generated-profile helper-order and runtime smoke checks for low-risk installed `windo` invocations.

## [8.5.3] - 2026-05-25

Release notes: [`docs/releases/RELEASE_NOTES_v8.5.3.md`](docs/releases/RELEASE_NOTES_v8.5.3.md).

### Fixed

- Bootstrap web helpers now omit `-OutFile` when no output path is supplied instead of passing an empty string.
- Bootstrap environment reads now tolerate missing variables under strict mode.
- Raw fallback downloads now run directly instead of through a background job that can miss parent-scope helper functions.
- Installed profile update helpers received the same environment-read hardening and direct download behavior.

## [8.5.2] - 2026-05-25

Release notes: [`docs/releases/RELEASE_NOTES_v8.5.2.md`](docs/releases/RELEASE_NOTES_v8.5.2.md).

### Fixed

- Hardened bootstrap retry classification so strict mode does not fail when an exception has no `Response` property.
- Hardened generated profile source checks with the same strict-mode-safe response handling.
- Installer now refuses to overwrite `$PROFILE` if the existing profile cannot be read or repaired.

### Added

- Managed profile block v2 metadata lines for clearer begin/end boundaries.
- Timestamped profile backups before WINDO rewrites the managed block.
- `profile.d` loader for user-owned customizations under `Documents\windo\profile.d` and `.pwsh_secure\profile.d`.

## [8.5.1] - 2026-05-25

Release notes: [`docs/releases/RELEASE_NOTES_v8.5.1.md`](docs/releases/RELEASE_NOTES_v8.5.1.md).

### Fixed

- Corrected generated profile help-token parsing from `- ieq` to the valid PowerShell operator `-ieq`, fixing profile load failures after update.
- Added installer-side profile parser validation so WINDO refuses to write an invalid generated profile block.
- Added regression tests that parse the installed profile block generated from installer here-strings.
