...
$script:WindoBootstrapExpectedVersion = "8.5.9"

function Test-WindoBootstrapDirectFileInvocation {
    param([AllowNull()][object]$InvocationInfo)

    if ($null -eq $InvocationInfo) { return $false }
    try {
        $pathProperty = $InvocationInfo.MyCommand.PSObject.Properties['Path']
        if ($null -eq $pathProperty -or [string]::IsNullOrWhiteSpace([string]$pathProperty.Value)) { return $false }
        return ([string]$InvocationInfo.InvocationName -ne '.')
    } catch {
        return $false
    }
}

$script:WindoBootstrapIsFileInvocation = Test-WindoBootstrapDirectFileInvocation -InvocationInfo $MyInvocation

# Windows PowerShell 5.1 can otherwise negotiate an obsolete protocol when
# reaching GitHub on machines with older .NET defaults.
try {
...
function Complete-WindoBootstrapInvocation {
    param([int]$Code)
    Set-WindoBootstrapExitCode $Code
    if ($script:WindoBootstrapIsFileInvocation) {
        exit $Code
    }
}
...
function Get-WindoBootstrapReleaseRef {
    ...
    if (Test-WindoBootstrapHasText $releaseCommitEnv) {
        ...
        Write-WindoBootstrapStep -Status warn -Label "WINDO_RELEASE_COMMIT invalid" -Detail "Ignoring the invalid override and resolving branch '$(Get-WindoBootstrapReleaseBranch)' to an immutable commit." -Color Yellow
    }
    ...
    for ($attempt = 1; $attempt -le 3; $attempt++) {
        try {
            ...
            if ($obj.sha -and ([string]$obj.sha -match '^[a-fA-F0-9]{40}$')) {
                $script:_windo_bootstrap_release_ref = ([string]$obj.sha).ToLowerInvariant()
                return $script:_windo_bootstrap_release_ref
            }
            throw "GitHub commit response did not include a valid commit SHA."
        } catch {
    ...
    throw "WINDO bootstrap could not resolve branch '$branch' to an immutable 40-character commit and will not download from a moving branch. Confirm access to api.github.com, retry, or set WINDO_RELEASE_COMMIT to a trusted commit. Last error: $resolutionError"
}
...
function _windo_bootstrap_is_retryable_web_error {
    ...
    if ($m -match 'timeout|timed out|temporarily unavailable|name resolution|name not known|No such host|connection (?:was )?(?:aborted|closed|reset)|request was aborted|unexpected (?:EOF|end of file)|SSL connection could not be established|Could not establish trust relationship|The remote name could not be resolved') { return $true }
...
function Invoke-WindoBootstrapDownload {
    ...
    for ($attempt = 1; $attempt -le 3; $attempt++) {
        try {
            ...
            if (-not (Test-Path -LiteralPath $OutFile) -or (Get-Item -LiteralPath $OutFile).Length -le 0) {
                throw "Download completed without a usable output file."
            }
            return
        } catch {
            $lastError = $_
    ...
    throw "Download failed after retry attempts: $($lastError.Exception.Message)"
}
...
function ConvertTo-WindoBootstrapInstallerArgumentLine {
    param([Parameter(Mandatory)][string]$ScriptPath)
    if ([string]::IsNullOrWhiteSpace($ScriptPath)) { throw "Installer path is empty." }
    if ($ScriptPath.Contains('"')) { throw "Installer path contains an unsupported quote character." }
    return '-NoProfile -ExecutionPolicy Bypass -File "' + $ScriptPath + '"'
}

function Test-WindoBootstrapUacCancellation {
    param([System.Management.Automation.ErrorRecord]$ErrorRecord)
    if ($null -eq $ErrorRecord -or $null -eq $ErrorRecord.Exception) { return $false }
    $exception = $ErrorRecord.Exception
    for ($depth = 0; $depth -lt 8 -and $null -ne $exception; $depth++) {
        try {
            $nativeCode = $exception.PSObject.Properties['NativeErrorCode']
            if ($null -ne $nativeCode -and [int]$nativeCode.Value -eq 1223) { return $true }
        } catch { }
        try { $exception = $exception.InnerException } catch { $exception = $null }
    }
    return ([string]$ErrorRecord.Exception.Message -match '(?i:operation|request).*(?:cancel(?:ed|led)|declined).*(?:user)?')
}
...
function Start-WindoBootstrapInstaller {
    ...
    $nativeArgumentLine = ConvertTo-WindoBootstrapInstallerArgumentLine -ScriptPath $ScriptPath
    $shouldElevate = [Environment]::UserInteractive

    if ($shouldElevate) {
        Write-Host "[windo] Requesting elevation for installer..." -ForegroundColor Cyan
        try {
            $installerProcess = Start-WindoBootstrapProcess -FilePath $runnerExe -Verb RunAs -ArgumentList $nativeArgumentLine -Wait -PassThru
            return [int]($installerProcess.ExitCode)
        } catch {
            if (Test-WindoBootstrapUacCancellation $_) {
                Write-Host "[windo] Installer launch was blocked: UAC consent was denied." -ForegroundColor Yellow
                Write-Host "  Re-run the same bootstrap command from a normal PowerShell window and approve UAC." -ForegroundColor Yellow
                return 1223
            }
            throw
        }
    ...
    throw "A complete unattended install requires an interactive UAC handoff. Run bootstrap from a normal interactive PowerShell window, or run a trusted local windo_install.ps1 from a pre-elevated automation context."
}
...
try {
    ...
    Complete-WindoBootstrapInvocation 1
    return
}
...
try {
    ...
    if (-not (ConvertFrom-WindoBootstrapBool -Name WINDO_SKIP_INSTALLER_SHA256)) {
        ...
        if ($null -eq $expected -or $expected.status -ne "available" -or -not $expected.sha256) {
            if (-not $bootstrapAttestedByBlob) {
                $detail = if ($null -ne $expected -and $expected.error) { $expected.error } else { "published checksum was unavailable" }
                throw "Installer SHA256 could not be verified and no GitHub blob attestation was available. Retry from a trusted network. Explicit emergency override: WINDO_SKIP_INSTALLER_SHA256=1. $detail"
            }
            Write-WindoBootstrapStep -Status warn -Label "Checksum source unavailable" -Detail "Installer bytes still match the commit-pinned GitHub blob object." -Color Yellow
            Write-WindoBootstrapStep -Status ok -Label "Installer integrity verified" -Detail "GitHub blob $($downloadSource.BlobSha)" -Color Green
        } else {
            $expectedSha = ([string]$expected.sha256).ToUpperInvariant()
            $got = Get-WindoBootstrapFileHash -Path $Temp
            if ($got -cne $expectedSha) {
                throw "Installer SHA256 does not match the commit-pinned published manifest. Ref=$(Get-WindoBootstrapReleaseRef) Expected=$expectedSha Got=$got"
            }
            Write-WindoBootstrapStep -Status ok -Label "Installer checksum verified" -Detail $got -Color Green
        }
    } else {
    ...
    if (-not $doLaunch) {
        ...
        if ($installerExitCode -eq 1223) {
            ...
            Write-Host "  Re-run this bootstrap command from a normal PowerShell window and approve UAC." -ForegroundColor DarkGray
...
Complete-WindoBootstrapInvocation $global:WINDO_EXIT_CODE
