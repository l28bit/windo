<# =====================================================================
WINDO v4.3.0 Special Edition Installer
...
$WindoBuiltinVerbs = @(
    ...
    'theme', 'output', 'motion', 'surface', 'control', 'upgrade', 'install-latest', 'uninstall', 'remove', 'profile', 'keybindings', 'completion', 'roadmap', 'syntax', 'mesh',
...
"@
    ...
    function _windo_new_command_plan([object[]]$Parts) {
        ...
        switch ($verb) {
            ...
            { $_ -in @("version", "config", "context", "profile", "roadmap", "preflight", "dashboard", "integrity", "verify", "session", "log", "history", "stats", "backups", "modules", "extras", "prompt", "ai", "help", "motion", "surface", "control") } {
                ...
                if ($verb -in @("dashboard", "export", "prompt", "extras", "backups", "modules", "control") -and (@($target) -match "fetch|enable|disable|prune|--html|--export|prime|queue|run|clear").Count -gt 0) {
                    $plan.writesLocalFiles = $true
                }
                [void]$next.Add($commandLine)
                break
    ...
    function _windo_launchpad_tray_script_text {
        $lines = @(
            ...
            '$notify.BalloonTipTitle = "WINDO Launchpad"',
            '$notify.BalloonTipText = "Special Edition tray command center is running. Double-click the WINDO icon or right-click for actions."',
            '$notify.BalloonTipIcon = [System.Windows.Forms.ToolTipIcon]::Info',
    ...
    function _windo_control_queued_requests {
        ...
        return @(Get-ChildItem -LiteralPath $queueRoot -Filter "*.json" -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | ForEach-Object {
            try {
    ...
    function _windo_control_state {
        ...
        [pscustomobject]@{
            ...
            nextCommands = @("windo control prime", "windo control actions", "windo control queue surface-prime", "windo control run launchpad-tray")
            exitCode = 0
        }
    }
    ...
    function _windo_help_topics {
        return @(
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo control [status] [--json]", "windo control prime [--json]", "windo control actions", "windo control queue <action-id>", "windo control run <action-id>", "windo control pulse", "windo control clear")
                Description = "Builds a local action catalog, manifest, and request queue under .pwsh_secure so tray/native surfaces can orchestrate known commands without a browser."
                Notes       = "Run launches only curated WINDO actions in a visible shell; queued requests remain explicit JSON artifacts until acted on."
                Examples    = @("windo control", "windo control prime", "windo control actions", "windo control queue surface-prime", "windo control run launchpad-tray")
            },
            [pscustomobject]@{
                Name        = "backups"
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "control") {
        ...
        if ($sub -notin @("status", "")) {
            if ($JsonOutput) { _emit_json "control" @{ error = "expected status | prime | actions | queue | run | pulse | clear"; exitCode = 2 } }
            else { Write-Host "[windo] control: expected status | prime | actions | queue | run | pulse | clear" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        $state = _windo_control_state
...
function __windo_completion_specs {
    @{
        help = @('version','install-latest','source','trust','scan','vault','sshx','crypto','explain','syntax','mesh','completion','output','motion','surface','control','keybindings','recipes','venv','pkg','launchpad','preflight','dashboard','integrity','verify','config','profile','modules','extras','ai','repair')
        ...
        control = @('status','prime','actions','queue','run','pulse','demo','clear','surface-status','surface-prime','launchpad-tray','workbench-html','motion-pulse','profile-doctor','trust-online','preflight','install-latest','--json')
