...
function Assert-WindoHostCompatibility {
    $profile = Get-WindoRuntimeProfile
    Write-WindoInstallStep -Status run -Label "Runtime preflight" -Detail "PS $($profile.PSVersion) / Windows $($profile.IsWindows10OrLater) / arch=$($profile.Environment)"
    ...
    if (-not $profile.SupportsBackgroundJobs) {
    ...
    $script:WindoScheduledTasksAvailable = $profile.ScheduledTasksAvailable
    ...
    if (-not $profile.WindowsFormsDrawingAvailable) {
    ...
    if (-not $profile.PSReadLineAvailable) {
    ...
    $script:WindoWindowsFormsDrawingAvailable = [bool]$profile.WindowsFormsDrawingAvailable
    return $profile
}
...
function Get-WindoEditionLabel {
    param([string]$Semver = $WindoVersion)
    $parts = @($Semver.Split('.'))
...
function Backup-WindoProfile {
    ...
    $backupPath = Join-Path $backupRoot ("Microsoft.PowerShell_profile.ps1.$stamp.bak")
    Copy-Item -LiteralPath $Path -Destination $backupPath -Force
    return $backupPath
}

function Ensure-WindoProfileD {
    foreach ($dir in @($ProfileDPublicDir, $ProfileDSecureDir)) {
        if (!(Test-Path -LiteralPath $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
    }
    $readme = Join-Path $ProfileDPublicDir "README.md"
    if (!(Test-Path -LiteralPath $readme)) {
        Write-Utf8NoBomFile -Path $readme -Content @"
# WINDO profile.d

Place local PowerShell profile customizations in `*.ps1` files here instead of editing the managed WINDO block in `$PROFILE`.

WINDO loads files from:

- `$HOME\Documents\windo\profile.d`
- `$HOME\.pwsh_secure\profile.d`

Files are loaded alphabetically. Parse errors and runtime errors are caught and reported as warnings so one custom file does not prevent the WINDO profile block from loading.
"@
    }
}

function Test-WindoProfileSyntax {
    param(
        [Parameter(Mandatory=$true)][string]$Content,
        [Parameter()][string]$Path = "<profile>"
    )
    ...
    if ($errors -and $errors.Count -gt 0) {
        $summary = @($errors | Select-Object -First 8 | ForEach-Object {
            $line = 0
            $column = 0
            try {
            ...
            if ($line -gt 0) { "{0}:{1}: {2}" -f $line, $column, $_.Message } else { [string]$_.Message }
        })
        $detail = $summary -join " | "
        if ($errors.Count -gt 8) { $detail += " | ... plus $($errors.Count - 8) more parser error(s)" }
        throw "Refusing to write invalid PowerShell profile '$Path'. Generated profile failed parser validation: $detail"
    }
    return $true
}

function Get-WindoErrorResponse {
...
$Manifest = [ordered]@{
    ...
    runnerLifecycle = [ordered]@{
        secureDir = $SecureDir
        runnerLast = $RunnerLast
        updateLast = $UpdateLast
        cleanupCommand = "windo runner cleanup --dry-run"
        repairCommand = "windo install-latest"
        configCommand = "windo runner config"
        doctorCommand = "windo runner doctor"
    }
...
function Invoke-WindoBundledUninstall {
    ...
    $UnUrl = "https://raw.githubusercontent.com/l28bit/windo/$branch/windo_uninstall.ps1"
    $TempUn = $null
    $ScriptPath = $null
    ...
    if ($Command -and $Command.Count -gt 0) {
        ...
        if ($tx -eq "/?" -or $tx -eq "?") {
                ...
                if (($leading + 1) -lt $rawCommand.Count) {
                    ...
                    if ($next -notlike '-*' -and $next -ne '/?' -and $next -ne "?") {
                        $HelpTopic = $next
                        $leading += 2
                        continue
                    }
        ...
        if ($Command.Count -ge 1 -and $Command[0] -eq "/?") { $HelpRequested = $true; if ($Command.Count -gt 1 -and $Command[1] -notlike '-*' -and $Command[1] -ne "?" -and $Command[1] -ne "/?" -and $Command[1] -ne "-?") { $HelpTopic = [string]$Command[1] }; $Command = if ($Command.Count -gt 1) { @($Command[1..($Command.Count - 1)]) } else { @() } }
        if ($Command.Count -ge 1 -and $Command[0] -eq "help" -and -not $HelpRequested) { if ($Command.Count -eq 1) { $HelpRequested = $true; $HelpTopic = "help" } }
        $normalized = if ($Command.Count -eq 1) { [string]$Command[0].ToLowerInvariant().Trim() } else { "" }
        if ($normalized -in @("?", "/?", "-?")) { $HelpRequested = $true; $HelpTopic = if ($HelpRequested) { "help" } else { "" }; $Command = @() }
            if ($Command.Count -ge 1 -and (($Command[-1] -ieq "/?") -or ($Command[-1] -ieq "?") -or ($Command[-1] -ieq "-?"))) {
        ...
        if ($Command.Count -gt 0) {
            ...
            if ($firstToken -eq "up") {
                $Command[0] = "install-latest"
            }
            elseif ($firstToken -eq "latest") {
                $Command[0] = "install-latest"
            }
            elseif ($firstToken -eq "install" -and $Command.Count -gt 1 -and [string]$Command[1].ToLowerInvariant().Trim() -eq "latest") {
                if ($Command.Count -gt 2) { $Command = @("install-latest") + @($Command[2..($Command.Count - 1)]) } else { $Command = @("install-latest") }
            }
            elseif ($firstToken -eq "self" -and $Command.Count -gt 1 -and [string]$Command[1].ToLowerInvariant().Trim() -eq "update") {
                if ($Command.Count -gt 2) { $Command = @("install-latest") + @($Command[2..($Command.Count - 1)]) } else { $Command = @("install-latest") }
            }
            elseif ($firstToken -eq "upd") { $Command[0] = "install-latest" }
            elseif ($firstToken -eq "pyenv") {
            ...
            elseif ($firstToken -eq "python") {
                if ($Command.Count -gt 1 -and ([string]$Command[1].ToLowerInvariant().Trim() -eq "venv")) {
                    if ($Command.Count -gt 2) { $Command = @("venv") + @($Command[2..($Command.Count - 1)]) } else { $Command = @("venv") }
                }
            }
            ...
            elseif ($firstToken -eq "dr") { $Command = @("runner", "doctor") + $(if ($Command.Count -gt 1) { @($Command[1..($Command.Count - 1)]) } else { @() }) }
...
function _windo_parse_runner_result([string]$OutPath, [string]$RequestId, [string]$Command, [int]$FallbackDurationMs) {
    ...
    function _windo_path_is_under_root {
        param(
            [string]$Path,
            [string]$Root
        )
    ...
    function _windo_file_fact {
        ...
        $level = if ($null -eq $ExpectedHash) { "UNKNOWN" } else { _integrity_component_level $actual $ExpectedHash }
        [pscustomobject]@{
            name = $Name
            path = $Path
    ...
    function _windo_task_fact {
        ...
        $author = $null
        $details = "missing"
        $remediation = "Run 'windo install-latest' from a standard shell and approve the UAC handoff, or run '.\windo_install.ps1' elevated from this repo."
        try {
            ...
            $details = "present; state=$state; enabled=$enabled"
        } catch {
        ...
        [pscustomobject]@{
            ...
            state = $state
            author = $author
            healthy = [bool]($present -and ($null -eq $enabled -or $enabled))
            details = $details
            remediation = $(if ($present) { "No repair required unless integrity or launch tests fail." } else { $remediation })
        }
    }

    function _windo_runner_cleanup_candidates {
        param([int]$OlderThanDays = 7)
        if ($OlderThanDays -lt 1) { $OlderThanDays = 7 }
        $cutoff = (Get-Date).AddDays(-1 * $OlderThanDays)
        $items = [System.Collections.ArrayList]@()
        if (-not (Test-Path -LiteralPath $SecureDir -PathType Container)) { return @($items) }
        $patterns = @("windo_req.*.json", "windo_res.*.json", ".windo_tmp_*.tmp", "windo_runner_*.tmp", "windo_update_*.tmp")
        foreach ($pattern in $patterns) {
            foreach ($item in @(Get-ChildItem -LiteralPath $SecureDir -Filter $pattern -File -ErrorAction SilentlyContinue)) {
                if ($item.LastWriteTime -gt $cutoff) { continue }
                $safe = _windo_path_is_under_root $item.FullName $SecureDir
                [void]$items.Add([pscustomobject]@{
                    ...
                    safeToDelete = [bool]$safe
                    reason = "older than $OlderThanDays day(s)"
                })
            }
        }
        return @($items)
    ...
    function _windo_runner_cleanup_execute {
        param(
            [switch]$Apply,
            [int]$OlderThanDays = 7
        )
        $candidates = @(_windo_runner_cleanup_candidates -OlderThanDays $OlderThanDays)
        $deleted = [System.Collections.ArrayList]@()
        $skipped = [System.Collections.ArrayList]@()
        foreach ($c in $candidates) {
            if (-not [bool]$c.safeToDelete) {
                [void]$skipped.Add([pscustomobject]@{ path = $c.path; reason = "outside secure root or unresolved path" })
                continue
            }
            if ($Apply) {
                try {
                    ...
                    [void]$skipped.Add([pscustomobject]@{ path = $c.path; reason = $_.Exception.Message })
                }
            }
        }
        [pscustomobject]@{
    ...
    function _windo_runner_lifecycle_state {
        param([int]$CleanupOlderThanDays = 7)
        $ix = _integrity_status
        $mainTask = _windo_task_fact -Name $TaskName -ManifestKey "main"
        $updateTask = _windo_task_fact -Name $TaskUpdate -ManifestKey "update"
        $cleanup = @(_windo_runner_cleanup_candidates -OlderThanDays $CleanupOlderThanDays)
        $files = [ordered]@{
            runner = (_windo_file_fact -Name "runner" -Path $RunnerPath -ExpectedHash $ix.RunnerExpected)
            selfUpdate = (_windo_file_fact -Name "self-update" -Path $UpdatePath -ExpectedHash $ix.UpdaterExpected)
            manifest = (_windo_file_fact -Name "manifest" -Path $ManifestFile)
            runnerLast = (_windo_file_fact -Name "runner-last" -Path $RunnerLast)
            updateLast = (_windo_file_fact -Name "update-last" -Path $UpdateLast)
            log = (_windo_file_fact -Name "audit-log" -Path $LogFile)
        }
        $ready = [bool]($mainTask.healthy -and $updateTask.healthy -and $files.runner.present -and $files.selfUpdate.present -and $ix.OverallLevel -eq "OK")
        $recommendations = [System.Collections.ArrayList]@()
        if (-not $mainTask.present -or -not $updateTask.present) { [void]$recommendations.Add("Repair task registration with 'windo install-latest' from a standard shell, then reload the profile.") }
        if (-not $files.runner.present -or -not $files.selfUpdate.present) { [void]$recommendations.Add("Runner assets are missing; run 'windo install-latest' to replace the WINDO profile block and secure-dir scripts.") }
        if ($ix.OverallLevel -ne "OK") { [void]$recommendations.Add("Run 'windo integrity' and reinstall from a verified installer if runner or updater hashes drift.") }
        if ($cleanup.Count -gt 0) { [void]$recommendations.Add("Review stale runner artifacts with 'windo runner cleanup --dry-run'; delete with '--apply' after review.") }
        if ($recommendations.Count -eq 0) { [void]$recommendations.Add("No runner repair action is required.") }
        [pscustomobject]@{
            schemaVersion = "1"
            windoVersion = $WindoVersion
            generatedAt = (Get-Date).ToString("o")
            state = $(if ($ready) { "ready" } elseif ($ix.OverallLevel -in @("TAMPERED", "DRIFT")) { "repair" } else { "attention" })
            ready = $ready
            secureDir = $SecureDir
            manifestFile = $ManifestFile
            tasks = [ordered]@{ main = $mainTask; update = $updateTask }
            files = $files
            integrity = [ordered]@{ overallLevel = $ix.OverallLevel; runnerLevel = $ix.RunnerLevel; updaterLevel = $ix.UpdaterLevel }
            config = [ordered]@{
                runnerTimeoutMs = (_windo_effective_runner_timeout_ms)
                runnerMaxCharsPerStream = (_windo_effective_runner_max_chars_per_stream)
                maxCommandChars = (_windo_max_command_chars)
                prefsFile = $PrefsFile
            }
            cleanup = [ordered]@{
                olderThanDays = [int]$CleanupOlderThanDays
                candidateCount = [int]$cleanup.Count
                dryRunCommand = "windo runner cleanup --dry-run --older-than $CleanupOlderThanDays"
                applyCommand = "windo runner cleanup --apply --older-than $CleanupOlderThanDays"
            }
            repair = [ordered]@{
                safeCommand = "windo install-latest"
                elevatedRepoCommand = ".\windo_install.ps1"
                hiddenExecution = $false
            }
            recommendations = @($recommendations)
            exitCode = $(if ($ready) { 0 } elseif ($ix.OverallLevel -eq "UNKNOWN") { 6 } else { 2 })
        }
    }
    ...
    function _windo_resolve_output_policy {
        ...
        if ($env:CI -and $mode -eq "legacy") { $mode = "compact" }
        $desc = switch ($mode) {
    ...
    function _windo_extras_index_url {
        ...
        $branch = _windo_release_branch
        return "https://raw.githubusercontent.com/l28bit/windo/$branch/extras/index.json"
    }
...
"@
    ...
    function _windo_roadmap_releases {
        return @(
            ...
            [pscustomobject]@{
                ...
                theme = "Harden tray, surface, motion, and native readiness before V8.4."
            ...
            [pscustomobject]@{
                version = "5.1.0"
                codename = "Command Center"
                theme = "Make the V8.4 command center feel complete and consistent, not just a command surface."
            ...
            [pscustomobject]@{
                version = "8.5.5"
                codename = "Bootstrap Handoff Guard"
                theme = "Keep the verified bootstrap-to-elevated-installer handoff clear of common-parameter collisions."
                focus = @("Start-Process wrapper hardening", "UAC handoff guardrail", "common parameter avoidance", "bootstrap launch regression coverage")
                status = "shipped"
                operatorValue = "Operators can approve the verified bootstrap handoff without the launch wrapper failing before UAC can complete."
            },
            [pscustomobject]@{
                version = "8.5.6"
                codename = "Dr. Run"
                theme = "Fortify the task runner lifecycle for download, repair, config, and cleanup workflows."
                focus = @("windo runner", "runner lifecycle model", "dry-run cleanup", "repair guidance", "config visibility", "safe cleanup invariant")
                status = "shipped"
                operatorValue = "Operators can inspect, repair-plan, configure, and clean the WINDO task runner without hidden execution or unsafe file deletion."
            },
            [pscustomobject]@{
                version = "8.5.7"
    ...
    function _windo_syntax_shortcuts {
        return @(
            ...
            [pscustomobject]@{
                id = "contract"
                aliases = @("release contract", "edition contract", "prometheus contract", "version contract")
                category = "Diagnostics"
                summary = "Inspect semver, edition, branch contract, and local alignment checks."
                command = "windo contract"
                preview = "windo contract doctor"
                risk = "read-only"
                notes = "Doctor mode adds published source alignment via network. Pair with windo version --contract for JSON-only contract fields."
            },
            [pscustomobject]@{
                id = "health"
    ...
    function _windo_new_command_plan([object[]]$Parts) {
        ...
        switch ($verb) {
            { $_ -in @("install-latest", "upgrade", "upd", "self-update") } {
        ...
        { $_ -in @("version", "config", "context", "profile", "roadmap", "preflight", "midflightfuel", "dashboard", "integrity", "verify", "session", "doctor", "runner", "log", "history", "stats", "backups", "modules", "extras", "prompt", "ai", "help", "motion", "surface", "control", "signal", "center", "health", "check", "status", "contract", "source") } {
                ...
                if ($verb -eq "runner") {
                    $plan.route = "task runner lifecycle"
                    $plan.checksumValidation = "runner/updater integrity is checked from the local manifest"
                    if ($sub -eq "cleanup") { $plan.writesLocalFiles = $true }
                    [void]$next.Add("windo runner doctor")
                    [void]$next.Add("windo runner cleanup --dry-run")
                }
                if ($verb -eq "midflightfuel") {
                    $plan.route = "curated preflight repair"
                    $plan.privilegeBoundary = "current user repairs plus explicit verified installer handoff when needed"
                    $plan.writesLocalFiles = $true
                    $plan.checksumValidation = "installer handoff verifies published installer before repair/update"
                    [void]$next.Add("windo preflight")
                    [void]$next.Add("windo midflightfuel --dry-run")
                }
    ...
    function _windo_run_published_installer {
        ...
        try {
            ...
            $runNow = $false
            $ciAutoMode = _windo_parse_bool_value $env:CI $false
            $nonInteractiveAuto = _windo_parse_bool_value $env:WINDO_INSTALL_NONINTERACTIVE $false
            if ($ForceContinue -or $nonInteractiveAuto -or $ciAutoMode) {
                $runNow = $true
                if ($nonInteractiveAuto -or $ciAutoMode) {
                    Write-Host "[windo] Proceeding without confirmation (automation/CI mode)." -ForegroundColor DarkGray
                }
            }
            if (-not $runNow -and $NonInteractive) {
            ...
            if (-not $runNow -and -not [Environment]::UserInteractive -and -not $ciAutoMode) {
            ...
            if (-not $runNow) {
                $defaultPrompt = "Run the installer now? (If approved, this same command relaunches elevated to register tasks.) [y/N]"
            ...
            if (-not $installerStarted) {
                Write-Host "[windo] ${DisplayCommand} launch was declined. Update handoff was not applied." -ForegroundColor Yellow
    ...
    function _read_last_meta {
        if ([string]::IsNullOrWhiteSpace($LastMetaFile)) { return $null }
    ...
    function _write_last_meta([string]$cmdLine, [string]$requestId) {
        if ([string]::IsNullOrWhiteSpace($LastMetaFile)) { return }
        $meta = @{
            schemaVersion = "1.0"
            commandLine = $cmdLine
    ...
    function _windo_normalize_handoff_prompt {
        ...
        $raw = if ($null -eq $PromptText) { "" } else { [string]$PromptText }
        $raw = $raw -replace '[\r\n\t]', ' '
        $raw = [regex]::Replace($raw, '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]', '')
        $raw = [regex]::Replace($raw, "`e\[[0-9;]*[ -/]*[@-~]", '')
        $raw = $raw -replace '\s{2,}', ' '
        $raw = $raw.Trim()
        if ([string]::IsNullOrWhiteSpace($raw)) { $raw = $DefaultPrompt }
        if ($raw -match '(?i)^\s*Input\s+content\b') { $raw = $DefaultPrompt }
        $raw = $raw -replace '(?i)\[y\s*/\s*n\]', '[y/N]'
        $raw = $raw -replace '\s{2,}', ' '
        if ($raw -notmatch '(?i)\[y\/n\]') { $raw = "$raw [y/N]" }
        $maxPromptChars = 220
        if ($raw.Length -gt $maxPromptChars) {
            $maxBody = [Math]::Max(1, $maxPromptChars - 8)
            $raw = "$($raw.Substring(0, $maxBody).TrimEnd()) [y/N]"
        }
        $raw = $raw.Trim()
        return $raw
    }
    ...
    function _windo_prompt_self_update_installer {
        ...
        $ciAutoMode = _windo_parse_bool_value $env:CI $false
        if ($NonInteractive -or $ciAutoMode -or -not [Environment]::UserInteractive) { return $false }

        $defaultPrompt = "Run the installer now? (If approved, this same command relaunches elevated to repair tasks.) [y/N]"
        $prompt = _windo_normalize_handoff_prompt -PromptText $env:SUDO_PROMPT -DefaultPrompt $defaultPrompt
    ...
    function _pretty_print([string]$cmdLine, [int]$exitCode, [string]$output, [int]$durationMs) {
        ...
        if ($policy.mode -eq "legacy") {
            ...
            $statusText = if ($exitCode -eq 0) { "SUCCESS" } else { "ERROR ($exitCode)" }
            $statusColor = if ($exitCode -eq 0) { "Green" } else { "Red" }
            Write-Host "[windo] Status: $statusText" -ForegroundColor $statusColor
        ...
        Write-Host ("[windo] {0} {1}ms :: {2} :: {3}" -f $statusText, $durationMs, $cmdLine, $outTag) -ForegroundColor $statusColor
        if (-not [string]::IsNullOrWhiteSpace($output)) { Write-Host $output }
        _suggest_if_denied $exitCode $output
    }
    ...
    function _windo_history_entry_matches {
        param(
            $Entry,
            [string]$Query,
            [switch]$UseRegex
        ...
        if ($null -ne $Entry.Command) { [void]$parts.Add([string]$Entry.Command) }
        if ($Entry.PSObject.Properties.Name -contains 'RequestId' -and $null -ne $Entry.RequestId) { [void]$parts.Add([string]$Entry.RequestId) }
        if ($Entry.PSObject.Properties.Name -contains 'User' -and $null -ne $Entry.User) { [void]$parts.Add([string]$Entry.User) }
        if ($Entry.PSObject.Properties.Name -contains 'Elevation' -and $null -ne $Entry.Elevation) { [void]$parts.Add([string]$Entry.Elevation) }
        $hay = ($parts -join ' ')
        if ($UseRegex) {
            try { return [regex]::IsMatch($hay, $Query, [System.Text.RegularExpressions.RegexOptions]::IgnoreCase) } catch { return $false }
        }
        return ($hay.IndexOf($Query, [StringComparison]::OrdinalIgnoreCase) -ge 0)
    }

    function _windo_filter_log_entries {
        param(
            [string]$Query,
            [int]$Limit = 25,
            [switch]$UseRegex,
        ...
        $all = @(_parse_log_entries -EmitWarnings $EmitWarnings)
        if ([string]::IsNullOrWhiteSpace($Query)) {
            return @($all | Select-Object -Last $Limit)
        }
        $matched = [System.Collections.ArrayList]@()
        for ($i = $all.Count - 1; $i -ge 0; $i--) {
            if (_windo_history_entry_matches -Entry $all[$i] -Query $Query -UseRegex:([bool]$UseRegex)) {
                [void]$matched.Insert(0, $all[$i])
...
function _windo_draw_ascii_startup_frame {
    ...
    function _windo_has_text {
        param([AllowNull()][string]$Value)
        return -not [string]::IsNullOrWhiteSpace($Value)
    }

    function _windo_get_error_response {
        param([System.Management.Automation.ErrorRecord]$ErrorRecord)
        if ($null -eq $ErrorRecord -or $null -eq $ErrorRecord.Exception) { return $null }
        try {
            $props = $ErrorRecord.Exception.PSObject.Properties
    ...
    function _windo_edition_label {
        $parts = @($WindoVersion.Split('.'))
        if ($parts.Count -ge 2) { return ("V{0}.{1}" -f $parts[0], $parts[1]) }
        return "V$WindoVersion"
    }

    function _windo_release_contract {
        $edition = _windo_edition_label
        return [ordered]@{
            semver = $WindoVersion
            edition = $edition
    ...
    function _windo_contract_posture {
        ...
        if (-not $profHasBlock) {
            [void]$checks.Add([pscustomobject]@{ id = "profile-block"; label = "Profile WINDO block"; level = "warn"; detail = "No WINDO block in current profile"; fixCommand = "windo install-latest" })
            if ($exitCode -lt 3) { $exitCode = 1 }
        } elseif ($null -eq $profVer) {
            [void]$checks.Add([pscustomobject]@{ id = "profile-version"; label = "Profile embedded version"; level = "warn"; detail = "Block present but version stamp unreadable"; fixCommand = "windo install-latest" })
            if ($exitCode -lt 3) { $exitCode = 1 }
        } elseif ($profVer -ne $WindoVersion) {
            [void]$checks.Add([pscustomobject]@{ id = "profile-version"; label = "Profile embedded version"; level = "warn"; detail = ("profile={0} installed={1}" -f $profVer, $WindoVersion); fixCommand = "windo install-latest" })
            if ($exitCode -lt 3) { $exitCode = 1 }
        } else {
            [void]$checks.Add([pscustomobject]@{ id = "profile-version"; label = "Profile embedded version"; level = "pass"; detail = $profVer; fixCommand = "" })
        }

        [void]$checks.Add([pscustomobject]@{ id = "semver"; label = "Installer semver"; level = "pass"; detail = $WindoVersion; fixCommand = "" })

        $ix = _integrity_status
        $ixLevel = if ($ix.OverallLevel -eq 'OK') { 'pass' } elseif ($ix.OverallLevel -eq 'UNKNOWN') { 'warn' } else { 'fail' }
        [void]$checks.Add([pscustomobject]@{ id = "integrity"; label = "Runner integrity"; level = $ixLevel; detail = ("overall={0}, runner={1}, updater={2}" -f $ix.OverallLevel, $ix.RunnerLevel, $ix.UpdaterLevel); fixCommand = $(if ($ixLevel -ne 'pass') { "windo integrity" } else { "" }) })
        if ($ixLevel -eq 'fail') { $exitCode = 3 } elseif ($ixLevel -eq 'warn' -and $exitCode -lt 3) { $exitCode = 1 }

        $branch = _windo_release_branch
        if ($branch -eq $contract.publishedContractBranch) {
            [void]$checks.Add([pscustomobject]@{ id = "release-branch"; label = "Release branch"; level = "pass"; detail = $branch; fixCommand = "" })
        } else {
            [void]$checks.Add([pscustomobject]@{ id = "release-branch"; label = "Release branch"; level = "warn"; detail = ("effective={0} published={1} (WINDO_TRACKING_BRANCH override)" -f $branch, $contract.publishedContractBranch); fixCommand = "" })
            if ($exitCode -lt 3) { $exitCode = 1 }
        }

        $source = $null
        if ($IncludeNetwork) {
            $source = _windo_source_status
            $srcLevel = if ([int]$source.exitCode -eq 0) { 'pass' } else { 'warn' }
            [void]$checks.Add([pscustomobject]@{ id = "published-source"; label = "Published source alignment"; level = $srcLevel; detail = $source.recommendation; fixCommand = $(if ($srcLevel -ne 'pass') { "windo install-latest" } else { "" }) })
            if ($srcLevel -ne 'pass' -and $exitCode -lt 3) { $exitCode = 1 }
        }

        $recommendation = "Contract posture is aligned."
        if ($exitCode -ge 3) { $recommendation = "Repair integrity or reinstall before privileged work." }
        elseif ($exitCode -ge 1) { $recommendation = "Refresh profile or published source alignment when convenient." }
    ...
    function _windo_release_ref {
        $releaseCommitEnv = _windo_get_env_value -Name "WINDO_RELEASE_COMMIT"
        if (_windo_has_text $releaseCommitEnv) {
            $candidate = [string]$releaseCommitEnv
            if ($candidate -match '^[a-fA-F0-9]{40}$') { return $candidate.ToLowerInvariant() }
            Write-Host ("[windo] WINDO_RELEASE_COMMIT is not a valid 40-hex commit; falling back to branch '{0}'." -f (_windo_release_branch)) -ForegroundColor Yellow
        }
    ...
    function _windo_is_retryable_web_error {
        ...
        $resp = _windo_get_error_response -ErrorRecord $ErrorRecord
        if ($null -ne $resp) {
            try {
    ...
    function _windo_new_check_row([string]$Id, [string]$Label, [bool]$Ok, [string]$Detail, [string]$FixCommand = "", [string]$Severity = "info", [string]$RepairAction = "") {
        return [pscustomobject]@{
            id = $Id
            label = $Label
            ok = [bool]$Ok
            ...
            repairCommand = $(if ([string]::IsNullOrWhiteSpace($RepairAction)) { "" } else { "windo midflightfuel --only $RepairAction" })
        }
    }
    ...
    function _windo_preflight_rows {
        ...
        [void]$rows.Add((_windo_new_check_row "shell-elevation" "Current shell is non-elevated" (-not $isElev) $(if ($isElev) { "High-privilege shell; remote update downloads are blocked by policy." } else { "Good for install-latest/bootstrap download safety." }) "Start a normal PowerShell window, then run: windo install-latest" $(if ($isElev) { "warn" } else { "info" }) "open-normal-shell"))

        $psMajor = 0
        try { $psMajor = [int]$PSVersionTable.PSVersion.Major } catch {}
        [void]$rows.Add((_windo_new_check_row "powershell-version" "PowerShell runtime" ($psMajor -ge 5) ("$($PSVersionTable.PSEdition) $($PSVersionTable.PSVersion)") "Install PowerShell 7 for best terminal experience: winget install Microsoft.PowerShell" $(if ($psMajor -ge 5) { "info" } else { "critical" }) "install-pwsh"))

        $mt = $false; $ut = $false
        try { Get-ScheduledTask -TaskName $TaskName -ErrorAction Stop | Out-Null; $mt = $true } catch {}
        try { Get-ScheduledTask -TaskName $TaskUpdate -ErrorAction Stop | Out-Null; $ut = $true } catch {}
        [void]$rows.Add((_windo_new_check_row "task-main" "Elevated runner task" $mt $(if ($mt) { "Task present: $TaskName" } else { "Missing task: $TaskName" }) "windo midflightfuel --only reinstall" $(if ($mt) { "info" } else { "critical" }) "reinstall"))
        [void]$rows.Add((_windo_new_check_row "task-update" "Self-update task" $ut $(if ($ut) { "Task present: $TaskUpdate" } else { "Missing task: $TaskUpdate" }) "windo midflightfuel --only reinstall" $(if ($ut) { "info" } else { "warn" }) "reinstall"))

        $ix = _integrity_status
        [void]$rows.Add((_windo_new_check_row "integrity" "Runner integrity" ($ix.OverallLevel -eq 'OK') ("overall=$($ix.OverallLevel), runner=$($ix.RunnerLevel), updater=$($ix.UpdaterLevel)") "windo midflightfuel --only reinstall" $(if ($ix.OverallLevel -eq 'OK') { "info" } else { "critical" }) "reinstall"))
        ...
        $policy = _windo_resolve_keybinding_policy
        [void]$rows.Add((_windo_new_check_row "keybindings" "Keybinding policy" ([bool]$policy.enabled) $(if ($policy.enabled) { "enabled chord=$($policy.chord), fallback=$($policy.fallbackChord)" } else { "disabled" }) "windo midflightfuel --only keybindings" $(if ($policy.enabled) { "info" } else { "warn" }) "keybindings"))

        return @($rows.ToArray())
    }

    function _windo_midflightfuel_plan {
        param([string[]]$Only = @())
        ...
        foreach ($item in @($Only)) {
            ...
            $wanted += [string]$item.Trim().ToLowerInvariant()
        }
        $actions = [System.Collections.ArrayList]@()
        foreach ($row in $failed) {
            $action = [string]$row.repairAction
            if ([string]::IsNullOrWhiteSpace($action)) { continue }
            if ($wanted.Count -gt 0 -and $wanted -notcontains $action -and $wanted -notcontains ([string]$row.id).ToLowerInvariant()) { continue }
            [void]$actions.Add([pscustomobject]@{
                ...
                command = [string]$row.repairCommand
            })
        }
        $unique = @($actions | Sort-Object id, checkId -Unique)
        [pscustomobject]@{
            generatedAt = (Get-Date).ToString("o")
            ...
            rescueCommand = "irm https://raw.githubusercontent.com/l28bit/windo/Exodus/bootstrap.ps1 | iex"
            exitCode = $(if ($unique.Count -gt 0) { 3 } elseif ($failed.Count -gt 0) { 2 } else { 0 })
        }
    }

    function _windo_midflightfuel_apply_action {
        param(
            [string]$ActionId,
            [switch]$DryRun,
            [switch]$Force
        )
        $id = if ([string]::IsNullOrWhiteSpace($ActionId)) { "" } else { $ActionId.Trim().ToLowerInvariant() }
        $result = [ordered]@{
            action = $id
            applied = $false
            ok = $false
        ...
        switch ($id) {
            "keybindings" {
                $result.command = "windo keybindings safe-reset"
                if ($DryRun) { $result.ok = $true; $result.message = "Would reset WINDO keybinding policy to safe defaults."; break }
                $res = _windo_keybindings_safe_reset_apply
                $result.applied = $true
                $result.ok = [bool]$res.ok
                $result.exitCode = $(if ($res.ok) { 0 } else { 2 })
                $result.message = $(if ($res.ok) { "Keybindings reset to safe defaults." } else { [string]$res.error })
                break
            }
            "audit-chain" {
                $result.command = "windo cleanup"
                if ($DryRun) { $result.ok = $true; $result.message = "Would back up and clear the active audit log so a clean chain can be rebuilt."; break }
                $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
                $backup = Join-Path $SecureDir ("windo_history.$stamp.enc.bak")
                try {
                    if (Test-Path -LiteralPath $LogFile) {
                        Copy-Item -LiteralPath $LogFile -Destination $backup -Force -ErrorAction Stop
                        Clear-Content -LiteralPath $LogFile -ErrorAction Stop
                        $result.message = "Backed up audit log to $backup and cleared active log."
                    } else {
                        $result.message = "No audit log exists; next logged command will create a fresh chain."
                    }
                    $result.applied = $true
                    $result.ok = $true
                } catch {
            ...
            "reinstall" {
                $result.command = "windo install-latest --force"
                if ($DryRun) { $result.ok = $true; $result.message = "Would download the verified installer and launch the repair/update handoff."; break }
                $started = _windo_run_published_installer -ForceContinue:$Force -DisplayCommand "midflightfuel"
                $result.applied = $true
                $result.ok = [bool]$started -and $global:WINDO_EXIT_CODE -eq 0
                $result.exitCode = [int]$global:WINDO_EXIT_CODE
                $result.message = $(if ($result.ok) { "Verified installer handoff completed." } else { "Installer repair handoff did not complete; try the bootstrap rescue command from a fresh normal shell." })
                break
            }
            "trust" {
                $result.command = "windo trust --online; windo integrity"
                if ($DryRun) { $result.ok = $true; $result.message = "Would re-check trust and trigger reinstall if trust remains REPAIR."; break }
                $trust = _windo_trust_posture $true
                if ($trust.trustLevel -eq "REPAIR") {
                    $startedTrust = _windo_run_published_installer -ForceContinue:$Force -DisplayCommand "midflightfuel"
                    $result.applied = $true
                    $result.ok = [bool]$startedTrust -and $global:WINDO_EXIT_CODE -eq 0
                    $result.exitCode = [int]$global:WINDO_EXIT_CODE
                    $result.message = "Trust posture required installer repair."
                } else {
                    $result.applied = $true
                    $result.ok = $true
                    $result.message = "Trust posture is $($trust.trustLevel); no installer repair required."
                }
                break
            }
            "open-normal-shell" {
                $result.command = "Start-Process pwsh.exe"
                if ($DryRun) { $result.ok = $true; $result.message = "Would open a fresh normal PowerShell window if possible."; break }
                try {
                    Start-Process -FilePath "pwsh.exe" -ArgumentList "-NoLogo" -WindowStyle Normal | Out-Null
                    $result.applied = $true
                    $result.ok = $true
                    $result.message = "Opened a fresh PowerShell window. Run windo preflight there."
                } catch {
                    $result.applied = $true
                    $result.ok = $false
                    $result.exitCode = 2
            ...
            "install-pwsh" {
                $result.command = "winget install Microsoft.PowerShell"
                $result.ok = $false
                $result.exitCode = 2
            ...
            default {
                $result.ok = $false
                $result.exitCode = 2
                $result.message = "Unknown midflightfuel action '$ActionId'."
            }
        }
    ...
    function _windo_operator_actions {
        return @(
            ...
            [pscustomobject]@{ title = "Launchpad"; command = "windo launchpad --open"; note = "Open the WINDO command center." },
            [pscustomobject]@{ title = "Contract Doctor"; command = "windo contract doctor"; note = "Release contract posture with published source alignment." },
    ...
    function _windo_edition_state {
        ...
        [pscustomobject]@{
            windoVersion = $WindoVersion
            edition = ("WINDO {0} Command Center" -f (_windo_edition_label))
            generatedAt = (Get-Date).ToString("o")
            branch = _windo_release_branch
            assets = $assets
            center = $center
    ...
    function _windo_write_edition_html([object]$Edition, [string]$OutputPath, [bool]$Open) {
        ...
        $centerTitle = [string]$Edition.edition
        $null = $sb.AppendLine(("<!doctype html><html><head><meta charset='utf-8'><title>{0}</title>" -f (_html_escape $centerTitle)))
        ...
        $null = $sb.AppendLine(("<section class='hero'><div><div class='eyebrow'>{0}</div>{1}{2}<div class='title'>{3}</div><div class='sub'>A local command surface for deliberate elevation, visible action routing, Signal Deck evidence, and native Windows tray flow.</div></div><div>{4}{5}</div></section>" -f (_html_escape $Edition.edition), $banner, $logo, (_html_escape $centerTitle), $avatar, $badge))
        ...
        $null = $sb.AppendLine(("<div class='card'><div class='k'>Branch</div><div class='v'>{0}</div><div class='muted'>{1}</div></div>" -f (_html_escape $Edition.branch), "v$([string]$Edition.windoVersion)"))
...
if ($changed) { Write-TextFileAtomic -Path $path -Content $body -Encoding ([System.Text.UTF8Encoding]::new($false)) }
    ...
    function _windo_control_action_catalog {
        return @(
            ...
            [pscustomobject]@{ id = "contract-doctor"; title = "Contract Doctor"; command = "windo contract doctor"; group = "trust"; execution = "visible-shell"; description = "Run release contract posture checks including published source alignment." },
            ...
            [pscustomobject]@{ id = "install-latest"; title = "Install Latest"; command = "windo install-latest"; group = "lifecycle"; execution = "visible-shell"; description = "Run the WINDO V8.4 installer/update handoff." }
        )
    }
    ...
    function _windo_control_set_request_status([object]$Found, [string]$Status, [hashtable]$Extra = $null) {
        ...
        if (Get-Command _windo_control_write_request -ErrorAction SilentlyContinue) {
            _windo_control_write_request $Found $request
        }
        if (Get-Command _windo_control_find_request -ErrorAction SilentlyContinue) {
...
"@
    ...
    function _windo_control_start_action([string]$ActionId) {
        ...
        $queue = _windo_control_queue_action ([string]$action.id)
        if (-not $queue.ok) {
            return [pscustomobject]@{ ok = $false; error = "unable to queue action"; actionId = [string]$action.id; command = [string]$action.command }
        }
        $request = $queue.request
        if ($null -eq $request) {
            return [pscustomobject]@{ ok = $false; error = "queued request missing"; actionId = [string]$action.id; command = [string]$action.command; exitCode = 2 }
        }
        $requestId = ([string]$request.id).Trim()
        if ([string]::IsNullOrWhiteSpace($requestId)) {
            $requestId = "cp-" + (Get-Date).ToString("yyyyMMddHHmmssfff") + "-" + ([Guid]::NewGuid().ToString("n").Substring(0, 6))
        }
        $global:WINDO_LAST_REQUEST_ID = $requestId
        $launchCommand = "windo control run $($action.id)"
        $actionCommand = [string]$action.command
        if ([string]::IsNullOrWhiteSpace($actionCommand)) {
            return [pscustomobject]@{ ok = $false; error = "empty control action command"; actionId = [string]$action.id; command = $null; requestId = $requestId; exitCode = 2 }
        }
        $exe = "powershell.exe"
        $pwsh = Get-Command pwsh.exe -ErrorAction SilentlyContinue
        if ($pwsh -and $pwsh.Source) { $exe = [string]$pwsh.Source }
        $encodedCommand = [Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($actionCommand))
        try {
            $process = Start-Process -FilePath $exe -ArgumentList @("-NoProfile", "-ExecutionPolicy", "Bypass", "-EncodedCommand", $encodedCommand) -PassThru -NoNewWindow
        } catch {
            return [pscustomobject]@{ ok = $false; error = "failed to launch action executor: $($_.Exception.Message)"; actionId = [string]$action.id; command = [string]$action.command; requestId = $requestId; exitCode = 2 }
        }
        ...
        $hasHasExited = $false
        if ($null -ne $process) {
            if ($process.PSObject.Properties.Name -contains "HasExited") {
                $hasHasExited = $true
                $hasExited = [bool]$process.HasExited
            }
            if (($process.PSObject.Properties.Name -contains "ExitCode") -and ($hasExited -or -not $hasHasExited)) {
        ...
        $metadata = [ordered]@{
            ...
            status = $(if ($hasExited -or -not $hasHasExited) { if ([int]$exitCode -eq 0 ) { "complete" } else { "failed" } } else { "running" })
            processId = if ($null -ne $process -and $process.PSObject.Properties.Name -contains "Id") { [int]$process.Id } else { $null }
        }
        if ($hasExited -or -not $hasHasExited) { $metadata["completedAt"] = (Get-Date).ToString("o") }
        if (Get-Command _windo_control_set_request_status -ErrorAction SilentlyContinue) {
            _windo_control_set_request_status $queue ([string]$metadata.status) $metadata
        }
        _append_log @{
            ...
            ExitCode = [int]$exitCode
            Output = "Started via control action"
            Elevation = "VISIBLE"
            DurationMs = 0
            Version = $WindoVersion
            actionId = [string]$action.id
        ...
        _write_last_meta $launchCommand $requestId
        return [pscustomobject]@{
            ok = $true
            actionId = [string]$action.id
            command = [string]$launchCommand
            ...
            exe = $exe
            processId = if ($null -ne $process -and $process.PSObject.Properties.Name -contains "Id") { [int]$process.Id } else { $null }
            hasExited = if ($null -ne $process) { [bool]$hasExited } else { $null }
            exitCode = [int]$exitCode
            requestId = [string]$requestId
        }
    }
    ...
    function _windo_help_topics {
        return @(
            [pscustomobject]@{
                ...
                Syntax      = @("windo help [topic]", "windo /? [topic]", "windo -? [topic]", "windo --help [topic]", "windo -h [topic]")
            ...
            [pscustomobject]@{
                ...
                Aliases     = @("upgrade", "upd")
                Category    = "Core"
                Summary     = "Download and run latest installer from V8.4."
            ...
            [pscustomobject]@{
                ...
                Description = "Uses the bundled local uninstaller when present, otherwise downloads it from V8.4, then starts an elevated uninstall."
        ...
        [pscustomobject]@{
            ...
            [pscustomobject]@{
                ...
                Description = "Displays the current V8.4 platform train and upcoming feature priorities without exposing future major-package details."
            ...
            [pscustomobject]@{
                Name        = "contract"
                Category    = "Core"
                Summary     = "Inspect release contract posture (semver, edition, branch, profile, integrity)."
                Syntax      = @("windo contract [--json]", "windo contract doctor [--json]", "windo contract --doctor [--json]")
                Description = "Local contract posture beyond raw version fields: profile block version stamp, integrity, and effective release branch. Doctor mode adds published source alignment."
                Notes       = "Use `windo version --contract` when you only need contract metadata without checks. Doctor performs read-only network calls like `windo source`."
                Examples    = @("windo contract", "windo contract doctor", "windo contract --json", "windo explain contract")
            },
            [pscustomobject]@{
                Name        = "source"
                Category    = "Core"
            ...
            [pscustomobject]@{
                ...
                Notes       = "The first V8.4 center is PowerShell-native. A compiled companion helper is scaffolded for a later major release but is not required."
                Examples    = @(
                    "windo center",
                    "windo center studio",
                    "windo center wizard",
                    "windo center power",
                    "windo center surface",
                    "windo center panel",
                    "windo center actions",
                    "windo center preview power-studio",
                    "windo center queue diagnostics-snapshot",
                    "windo center queue health-snapshot-html",
                    "windo center queue upgrade-history-open",
                    "windo center run open-windo-folder",
                    "windo center run diagnostics-snapshot",
                    "windo center run log-bundle-open",
                    "windo center run center-open",
                    "windo center execute-next",
                    "windo center signal"
                )
            },
            [pscustomobject]@{
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo doctor [--json]", "windo health [--json]", "windo health \[--json\]")
                Description = "Checks scheduled tasks, manifest presence, runner/log locations, and Dr. Run lifecycle state."
                Notes       = "Sets $global:WINDO_EXIT_CODE for machine checks. Use windo runner doctor for deeper task runner workflow detail."
                Examples    = @("windo doctor", "windo health", "windo doctor --json", "windo health --json")
            },
            [pscustomobject]@{
                Name        = "integrity"
                Category    = "Inspection"
            ...
            [pscustomobject]@{
                Name        = "history"
                Category    = "Inspection"
                Summary     = "Recent or filtered command history from encrypted audit."
                Syntax      = @("windo history [-n N]", "windo history --contains <text>", "windo history search <query> [-n N] [--regex] [--json]")
                Description = "Shows compact summaries for last N entries, or searches decrypted audit entries by command text (and request id when present)."
                Notes       = "Search scans the full log newest-first; large logs may take longer. Defaults: tail=50, search=25."
                Examples    = @("windo history", "windo history -n 20", "windo history search install-latest", "windo history --contains elevation", "windo history search 'windo run' -n 10 --json")
            },
            [pscustomobject]@{
                Name        = "report"
                Category    = "Reporting"
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo preflight [--json]", "windo check [--json]", "windo check \[--json\]")
                Description = "Checks non-elevated update posture, PowerShell runtime, scheduled tasks, integrity, audit chain, profile block, and keybinding policy. Non-OK findings point to midflightfuel repair lanes."
                Notes       = "Read-only; suggested fix commands are printed but not executed. Run windo midflightfuel to apply curated repairs."
                Examples    = @("windo preflight", "windo check", "windo check --json", "windo preflight --json")
            },
            [pscustomobject]@{
                Name        = "midflightfuel"
                Category    = "Maintenance"
                Summary     = "Apply curated repairs for non-OK preflight findings."
                Syntax      = @("windo midflightfuel [--dry-run] [--force] [--only <action|check-id>] [--json]")
                Description = "Reads current preflight findings and runs only supported repair lanes: reinstall handoff, keybinding reset, audit-chain cleanup, trust repair, normal-shell launch, or manual PowerShell install guidance."
                Notes       = "No arbitrary command execution. From a badly broken or very old profile, use the bootstrap rescue command printed by this command."
                Examples    = @("windo preflight", "windo midflightfuel --dry-run", "windo midflightfuel", "windo midflightfuel --only keybindings")
            },
            [pscustomobject]@{
                Name        = "launchpad"
                Category    = "Reporting"
            ...
            [pscustomobject]@{
                Name        = "cleanup"
                Category    = "Maintenance"
                Summary     = "Back up and clear active log; optionally clean stale runner artifacts."
                Syntax      = @("windo cleanup [-w] [--runner-dry-run] [--runner-apply] [--older-than N]")
                Description = "Creates windo_history.<timestamp>.enc.bak and clears log. Runner request/result/temp cleanup is dry-run unless --runner-apply is used."
                Notes       = "`-w` is accepted for compatibility and ignored. Runner cleanup only targets known WINDO patterns under .pwsh_secure."
                Examples    = @("windo cleanup", "windo cleanup -w", "windo cleanup --runner-dry-run", "windo cleanup --runner-apply --older-than 14")
            },
            [pscustomobject]@{
                Name        = "runner"
                Aliases     = @("dr")
                Category    = "Maintenance"
                Summary     = "Dr. Run task runner lifecycle, repair plan, config, and cleanup."
                Syntax      = @("windo runner [doctor|status|dr] [--json]", "windo runner config [--json]", "windo runner cleanup [--dry-run|--apply] [--older-than N] [--json]", "windo runner repair [--json]")
                Description = "Inspects scheduled task bridge, runner/updater files, manifest-backed integrity, effective runner config, and stale artifact cleanup candidates."
                Notes       = "Repair planning is explicit and visible; WINDO does not hide arbitrary command execution behind runner repair."
                Examples    = @("windo runner doctor", "windo runner config", "windo runner cleanup --dry-run", "windo runner cleanup --apply --older-than 14", "windo runner repair --json")
            },
            [pscustomobject]@{
                Name        = "modules"
                Category    = "Shell Experience"
            ...
            [pscustomobject]@{
                ...
                Description = "Downloads the published extras index from GitHub (V8.4) and searches entries. Fetch downloads artifacts only from a non-elevated shell and verifies SHA256 when published."
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo session [--json]", "windo status [--json]", "windo status \[--json\]")
                ...
                Examples    = @("windo session --json", "windo status --json")
            },
            [pscustomobject]@{
                Name        = "ai"
                Category    = "Shell Experience"
            ...
            [pscustomobject]@{
                ...
                Notes       = "For a full profile refresh from V8.4, run 'windo install-latest' from a non-elevated shell after repair."
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "contract") {
        $doctorMode = $false
        $badArgs = [System.Collections.ArrayList]@()
        for ($ci = 1; $ci -lt $Command.Count; $ci++) {
            $ca = [string]$Command[$ci]
            if ([string]::IsNullOrWhiteSpace($ca)) { continue }
            if ($ca -eq "doctor" -and -not $doctorMode) { $doctorMode = $true; continue }
            if ($ca -eq "--doctor") { $doctorMode = $true; continue }
            if ($ca -eq "--json") { continue }
            [void]$badArgs.Add($ca)
        }
        if ($badArgs.Count -gt 0) {
            if ($JsonOutput) { _emit_json "contract" @{ error = "unexpected argument"; invalidArgs = @($badArgs.ToArray()); exitCode = 2 } }
            else { Write-Host "[windo] contract: unexpected argument(s): $($badArgs -join ', ')" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        $posture = _windo_contract_posture -IncludeNetwork:([bool]$doctorMode)
        if ($JsonOutput) {
            _emit_json "contract" $posture
            _windo_set_exit ([int]$posture.exitCode)
            return
        }
        Write-Host "[windo] Release contract posture$(if ($doctorMode) { ' (doctor)' })" -ForegroundColor Cyan
        Write-Host ("  Semver   : {0} ({1})" -f $posture.contract.semver, $posture.contract.edition)
        Write-Host ("  Branch   : {0} (published={1})" -f $posture.contract.releaseBranch, $posture.contract.publishedContractBranch)
        Write-Host ("  Schema   : {0}" -f $posture.contract.schemaVersion)
        foreach ($chk in @($posture.checks)) {
            $color = switch ($chk.level) { 'pass' { 'Green' } 'warn' { 'Yellow' } 'fail' { 'Red' } default { 'Gray' } }
            Write-Host ("  [{0}] {1} : {2}" -f $chk.level.ToUpperInvariant(), $chk.label, $chk.detail) -ForegroundColor $color
            if (-not [string]::IsNullOrWhiteSpace($chk.fixCommand)) {
                Write-Host ("         fix: {0}" -f $chk.fixCommand) -ForegroundColor DarkGray
            }
        }
        Write-Host ("  Next     : {0}" -f $posture.recommendation) -ForegroundColor Yellow
        _windo_set_exit ([int]$posture.exitCode)
        return
    }
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "source") {
        ...
        Write-Host ("  Contract  : $($src.contract.semver) $($src.contract.edition), branch=$($src.contract.releaseBranch)") -ForegroundColor DarkGray
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "config") {
        ...
        $rows = [System.Collections.ArrayList]@(
            ...
            [pscustomobject]@{ name = "WINDO_SKIP_INSTALLER_SHA256"; environmentValue = $(if ($env:WINDO_SKIP_INSTALLER_SHA256) { [string]$env:WINDO_SKIP_INSTALLER_SHA256 } else { $null }); effectiveNote = $(if ($env:WINDO_SKIP_INSTALLER_SHA256) { "bootstrap/upgrade checksum check skipped" } else { "checksum enforced when published on V8.4" }) }
        ...
        if ($JsonOutput) {
            $runnerLifecycleForConfig = _windo_runner_lifecycle_state
            _emit_json "config" @{
                settings = @($rows)
                secureDir = $SecureDir
                runnerLifecycle = @{ state = $runnerLifecycleForConfig.state; ready = $runnerLifecycleForConfig.ready; config = $runnerLifecycleForConfig.config; cleanup = $runnerLifecycleForConfig.cleanup }
        ...
        $runnerLifecycleForText = _windo_runner_lifecycle_state
        Write-Host "  Runner   : state=$($runnerLifecycleForText.state), timeout=$($runnerLifecycleForText.config.runnerTimeoutMs)ms, outputCap=$($runnerLifecycleForText.config.runnerMaxCharsPerStream)" -ForegroundColor DarkGray
        foreach ($r in $rows) {
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "runner") {
        $sub = "doctor"
        if ($Command.Count -ge 2 -and -not ([string]$Command[1]).StartsWith("-")) { $sub = ([string]$Command[1]).Trim().ToLowerInvariant() }
        if ($sub -eq "dr") { $sub = "doctor" }
        if ($sub -eq "status") { $sub = "doctor" }
        if ($sub -notin @("doctor", "config", "cleanup", "repair")) {
            if ($JsonOutput) { _emit_json "runner" @{ error = "expected doctor, status, config, cleanup, repair, or dr"; subcommand = $sub; exitCode = 2 } }
            else { Write-Host "[windo] runner: expected doctor | status | config | cleanup | repair | dr" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        ...
        $applyCleanup = $false
        $olderThanDays = 7
        $badRunnerArgs = [System.Collections.ArrayList]@()
        $argStart = if ($Command.Count -ge 2 -and -not ([string]$Command[1]).StartsWith("-")) { 2 } else { 1 }
        for ($ri = $argStart; $ri -lt $Command.Count; $ri++) {
            $ra = [string]$Command[$ri]
            if ([string]::IsNullOrWhiteSpace($ra)) { continue }
            if ($ra -eq "--apply") { $applyCleanup = $true; continue }
            if ($ra -eq "--dry-run") { $applyCleanup = $false; continue }
            if ($ra -in @("--older-than", "--older-than-days", "--days")) {
                if ($ri + 1 -ge $Command.Count) { [void]$badRunnerArgs.Add("$ra requires a value"); continue }
                try { $olderThanDays = [int]$Command[$ri + 1] } catch { [void]$badRunnerArgs.Add("$ra value must be an integer") }
                $ri++
                continue
            }
            if ($ra -like "--older-than=*") {
                try { $olderThanDays = [int]$ra.Substring(13) } catch { [void]$badRunnerArgs.Add("--older-than value must be an integer") }
                continue
            }
            if ($ra -like "--older-than-days=*") {
                try { $olderThanDays = [int]$ra.Substring(18) } catch { [void]$badRunnerArgs.Add("--older-than-days value must be an integer") }
                continue
            }
            [void]$badRunnerArgs.Add($ra)
        }
        if ($olderThanDays -lt 1) { [void]$badRunnerArgs.Add("--older-than must be >= 1") }
        if ($badRunnerArgs.Count -gt 0) {
            if ($JsonOutput) { _emit_json "runner" @{ error = "invalid argument"; invalidArgs = @($badRunnerArgs.ToArray()); exitCode = 2 } }
            else { Write-Host "[windo] runner $($sub): invalid argument(s): $($badRunnerArgs -join ', ')" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }

        if ($sub -eq "cleanup") {
            $cleanupResult = _windo_runner_cleanup_execute -Apply:$applyCleanup -OlderThanDays $olderThanDays
            if ($JsonOutput) {
                _emit_json "runner" @{ subcommand = "cleanup"; cleanup = $cleanupResult; exitCode = [int]$cleanupResult.exitCode }
                _windo_set_exit ([int]$cleanupResult.exitCode)
                return
            }
            Write-Host ("[windo] Runner cleanup ({0}, older than {1} day(s))" -f $(if ($applyCleanup) { "apply" } else { "dry-run" }), $olderThanDays) -ForegroundColor Cyan
            Write-Host "  SecureDir  : $SecureDir" -ForegroundColor DarkGray
            Write-Host "  Candidates : $($cleanupResult.candidateCount)" -ForegroundColor $(if ($cleanupResult.candidateCount -gt 0) { "Yellow" } else { "Green" })
            if ($applyCleanup) {
                Write-Host "  Deleted    : $($cleanupResult.deletedCount)" -ForegroundColor $(if ($cleanupResult.deletedCount -gt 0) { "Yellow" } else { "DarkGray" })
                if ($cleanupResult.skippedCount -gt 0) { Write-Host "  Skipped    : $($cleanupResult.skippedCount)" -ForegroundColor Red }
            } else {
                Write-Host "  Apply with : windo runner cleanup --apply --older-than $olderThanDays" -ForegroundColor DarkGray
            }
            foreach ($c in @($cleanupResult.candidates | Select-Object -First 12)) {
                Write-Host ("  - {0}  {1} bytes  {2}" -f $c.name, $c.length, $c.reason) -ForegroundColor DarkGray
            }
            if ($cleanupResult.candidateCount -gt 12) { Write-Host "  ... $($cleanupResult.candidateCount - 12) more candidate(s)" -ForegroundColor DarkGray }
            Write-Host "  Invariant  : $($cleanupResult.invariant)" -ForegroundColor DarkGray
            _windo_set_exit ([int]$cleanupResult.exitCode)
            return
        }

        $life = _windo_runner_lifecycle_state -CleanupOlderThanDays $olderThanDays
        if ($sub -eq "config") {
            if ($JsonOutput) {
                _emit_json "runner" @{ subcommand = "config"; config = $life.config; paths = @{ secureDir = $SecureDir; manifestFile = $ManifestFile; runner = $RunnerPath; updater = $UpdatePath }; exitCode = 0 }
                _windo_set_exit 0
                return
            }
            Write-Host "[windo] Runner config" -ForegroundColor Cyan
            Write-Host "  Timeout       : $($life.config.runnerTimeoutMs) ms" -ForegroundColor DarkGray
            Write-Host "  Output cap    : $($life.config.runnerMaxCharsPerStream) chars per stream" -ForegroundColor DarkGray
            Write-Host "  Command cap   : $($life.config.maxCommandChars) chars" -ForegroundColor DarkGray
            Write-Host "  Prefs         : $($life.config.prefsFile)" -ForegroundColor DarkGray
            _windo_set_exit 0
            return
        }
        if ($sub -eq "repair") {
            if ($JsonOutput) {
                _emit_json "runner" @{ subcommand = "repair"; state = $life.state; repair = $life.repair; recommendations = @($life.recommendations); exitCode = [int]$life.exitCode }
                _windo_set_exit ([int]$life.exitCode)
                return
            }
            Write-Host "[windo] Runner repair plan" -ForegroundColor Cyan
            Write-Host "  State       : $($life.state)" -ForegroundColor $(if ($life.ready) { "Green" } else { "Yellow" })
            Write-Host "  Safe repair : $($life.repair.safeCommand)" -ForegroundColor Green
            Write-Host "  Repo repair : $($life.repair.elevatedRepoCommand) (from repo, elevated)" -ForegroundColor DarkGray
            Write-Host "  Hidden exec : $($life.repair.hiddenExecution)" -ForegroundColor DarkGray
            foreach ($rec in @($life.recommendations)) { Write-Host "  -> $rec" -ForegroundColor Yellow }
            _windo_set_exit ([int]$life.exitCode)
            return
        }

        if ($JsonOutput) {
            _emit_json "runner" @{ subcommand = "doctor"; lifecycle = $life; exitCode = [int]$life.exitCode }
            _windo_set_exit ([int]$life.exitCode)
            return
        }
        Write-Host "[windo] Dr. Run - task runner lifecycle" -ForegroundColor Cyan
        Write-Host "  State     : $($life.state)" -ForegroundColor $(if ($life.ready) { "Green" } elseif ($life.state -eq "repair") { "Red" } else { "Yellow" })
        Write-Host "  Integrity : $($life.integrity.overallLevel) (runner $($life.integrity.runnerLevel), self-update $($life.integrity.updaterLevel))"
        Write-Host "  Main task : $(if ($life.tasks.main.present) { $life.tasks.main.details } else { 'MISSING' })" -ForegroundColor $(if ($life.tasks.main.present) { "Green" } else { "Red" })
        Write-Host "  Upd task  : $(if ($life.tasks.update.present) { $life.tasks.update.details } else { 'MISSING' })" -ForegroundColor $(if ($life.tasks.update.present) { "Green" } else { "Red" })
        Write-Host "  Runner    : $(if ($life.files.runner.present) { $life.files.runner.level } else { 'MISSING' })  $RunnerPath" -ForegroundColor $(if ($life.files.runner.present -and $life.files.runner.level -eq "OK") { "Green" } else { "Yellow" })
        Write-Host "  Cleanup   : $($life.cleanup.candidateCount) stale candidate(s); $($life.cleanup.dryRunCommand)" -ForegroundColor DarkGray
        foreach ($rec in @($life.recommendations)) { Write-Host "  -> $rec" -ForegroundColor Yellow }
        _windo_set_exit ([int]$life.exitCode)
        return
    }
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "roadmap") {
        ...
        if ($JsonOutput) {
            ...
            Write-Host ("  Current : {0} ({1})" -f $WindoVersion, (_windo_edition_label)) -ForegroundColor DarkGray
            Write-Host ("  Target  : {0} contract posture (Wave 11+ operator surface)" -f (_windo_edition_label)) -ForegroundColor Green
...
"@
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "preflight") {
        ...
        $payload = @{
            ...
            checks = $rows
            repairCommand = $(if ($fail.Count -gt 0) { "windo midflightfuel" } else { $null })
            repairPlan = (_windo_midflightfuel_plan)
            exitCode = $exit
        }
        if ($JsonOutput) {
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "midflightfuel") {
        ...
        $badFuelArgs = [System.Collections.ArrayList]@()
        for ($fi = 1; $fi -lt $Command.Count; $fi++) {
            $fa = [string]$Command[$fi]
            if ([string]::IsNullOrWhiteSpace($fa)) { continue }
            if ($fa -eq "--dry-run" -or $fa -eq "-WhatIf") { $dryRunFuel = $true; continue }
            if ($fa -eq "--force" -or $fa -eq "-y") { $forceFuel = $true; continue }
            if ($fa -eq "--only") {
                if ($fi + 1 -ge $Command.Count) { [void]$badFuelArgs.Add("--only requires a value"); continue }
                [void]$onlyFuel.Add([string]$Command[$fi + 1])
                $fi++
                continue
            }
            if ($fa -like "--only=*") {
                [void]$onlyFuel.Add($fa.Substring(7))
                continue
            }
            [void]$badFuelArgs.Add($fa)
        }
        if ($badFuelArgs.Count -gt 0) {
            if ($JsonOutput) { _emit_json "midflightfuel" @{ error = "invalid argument"; invalidArgs = @($badFuelArgs.ToArray()); exitCode = 2 } }
            else { Write-Host "[windo] midflightfuel: invalid argument(s): $($badFuelArgs -join ', ')" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        $plan = _windo_midflightfuel_plan -Only @($onlyFuel.ToArray())
        $results = [System.Collections.ArrayList]@()
        if ($plan.actionCount -gt 0) {
            foreach ($action in @($plan.actions)) {
                $res = _windo_midflightfuel_apply_action -ActionId ([string]$action.id) -DryRun:$dryRunFuel -Force:$forceFuel
                [void]$results.Add($res)
                if (-not [bool]$res.ok -and [int]$res.exitCode -ne 0) { break }
            }
        }
        $failedFuel = @($results | Where-Object { -not $_.ok })
        $fuelExit = if ($plan.actionCount -eq 0 -and $plan.failedCount -eq 0) { 0 } elseif ($plan.actionCount -eq 0) { 2 } elseif ($failedFuel.Count -gt 0) { 2 } elseif ($dryRunFuel) { 0 } else { 0 }
        $payloadFuel = [ordered]@{
            windoVersion = $WindoVersion
            dryRun = [bool]$dryRunFuel
            force = [bool]$forceFuel
            plan = $plan
            results = @($results)
            rescueCommand = $plan.rescueCommand
            exitCode = $fuelExit
        }
        if ($JsonOutput) {
            _emit_json "midflightfuel" $payloadFuel
            _windo_set_exit $fuelExit
            return
        }
        Write-Host "[windo] midflightfuel" -ForegroundColor Cyan
        if ($plan.failedCount -eq 0) {
            Write-Host "  Preflight is already OK; no fuel needed." -ForegroundColor Green
        } elseif ($plan.actionCount -eq 0) {
            Write-Host "  No automatic repair lane matched the selected preflight findings." -ForegroundColor Yellow
        } else {
            Write-Host "  Mode    : $(if ($dryRunFuel) { 'dry-run' } else { 'apply' })" -ForegroundColor DarkGray
            Write-Host "  Actions : $($plan.actionCount)" -ForegroundColor DarkGray
            foreach ($r in @($results)) {
                $color = if ($r.ok) { "Green" } else { "Yellow" }
                Write-Host ("  [{0}] {1} - {2}" -f $(if ($r.ok) { "OK" } else { "!!" }), $r.action, $r.message) -ForegroundColor $color
                if (-not [string]::IsNullOrWhiteSpace([string]$r.command)) { Write-Host ("       command: {0}" -f $r.command) -ForegroundColor DarkGray }
            }
        }
        Write-Host "  Rescue  : $($plan.rescueCommand)" -ForegroundColor DarkGray
        _windo_set_exit $fuelExit
        return
    }
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "repair") {
        ...
        $hints = @(
            ...
            "From a normal (non-elevated) window:  windo install-latest  - refreshes the embedded profile from V8.4 when you are behind."
        )
        if ($JsonOutput) {
            _emit_json "repair" @{
        ...
        if ($Command.Count -gt 1) {
            foreach ($a in $Command[1..($Command.Count - 1)]) {
                ...
                if ($aa -eq '--force' -or $aa -eq '-Force') { $forceInst = $true }
                    ...
                    Write-Host ("[windo] {0}: unknown argument '{1}' (use --force to skip confirmation)" -f $requestedUpdateCommand, $aa) -ForegroundColor Yellow
                    _windo_set_exit 2
                    return
                }
            }
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "history") {
        ...
        $searchQuery = $null
        $useRegex = $false
        $badHistArgs = [System.Collections.ArrayList]@()
        if ($Command.Count -ge 2 -and [string]$Command[1] -eq "search") {
            $mode = "search"
            $hn = 25
            $hi = 2
            $queryParts = [System.Collections.ArrayList]@()
            while ($hi -lt $Command.Count) {
                $ht = [string]$Command[$hi]
                if ([string]::IsNullOrWhiteSpace($ht)) { $hi++; continue }
                if ($ht -eq "-n" -and ($hi + 1) -lt $Command.Count) {
                    try { $hn = [int]$Command[$hi + 1] } catch { $hn = 25 }
                    $hi += 2
                    continue
                }
                if ($ht -eq "-Regex" -or $ht -eq "--regex") { $useRegex = $true; $hi++; continue }
                if ($ht.StartsWith("--")) { [void]$badHistArgs.Add($ht); $hi++; continue }
                [void]$queryParts.Add($ht)
                $hi++
            }
            if ($badHistArgs.Count -gt 0) {
                if ($JsonOutput) { _emit_json "history" @{ error = "unexpected argument"; invalidArgs = @($badHistArgs.ToArray()); exitCode = 2 } }
                else { Write-Host "[windo] history search: unexpected argument(s): $($badHistArgs -join ', ')" -ForegroundColor Yellow }
                _windo_set_exit 2
                return
            }
            $searchQuery = (($queryParts | ForEach-Object { [string]$_ }) -join ' ').Trim()
            if ([string]::IsNullOrWhiteSpace($searchQuery)) {
                if ($JsonOutput) { _emit_json "history" @{ error = "search query required"; exitCode = 2 } }
                else { Write-Host "[windo] history search: provide a query (example: windo history search install-latest)" -ForegroundColor Yellow }
                _windo_set_exit 2
                return
            }
        } else {
            $ti = 1
            while ($ti -lt $Command.Count) {
                $ht = [string]$Command[$ti]
                if ([string]::IsNullOrWhiteSpace($ht)) { $ti++; continue }
                if ($ht -eq "-n" -and ($ti + 1) -lt $Command.Count) {
                    try { $hn = [int]$Command[$ti + 1] } catch { $hn = 50 }
                    $ti += 2
                    continue
                }
                if ($ht -eq "--contains" -and ($ti + 1) -lt $Command.Count) {
                    $mode = "search"
                    $searchQuery = [string]$Command[$ti + 1]
                    $hn = 25
                    $ti += 2
                    continue
                }
                if ($ht.StartsWith("--") -and $ht -ne "--json") { [void]$badHistArgs.Add($ht); $ti++; continue }
                if ($mode -ne "search") { [void]$badHistArgs.Add($ht); $ti++; continue }
                $ti++
            }
            if ($badHistArgs.Count -gt 0) {
                if ($JsonOutput) { _emit_json "history" @{ error = "unexpected argument"; invalidArgs = @($badHistArgs.ToArray()); exitCode = 2 } }
                else { Write-Host "[windo] history: unexpected argument(s): $($badHistArgs -join ', ')" -ForegroundColor Yellow }
                _windo_set_exit 2
                return
            }
        }
        ...
        if ($mode -eq "search") {
            $slice = @(_windo_filter_log_entries -Query $searchQuery -Limit $hn -UseRegex:([bool]$useRegex) -EmitWarnings $false)
            if ($JsonOutput) {
                _emit_json "history" @{ mode = "search"; query = $searchQuery; useRegex = [bool]$useRegex; entries = $slice; count = $slice.Count; exitCode = 0 }
                return
            }
            Write-Host "[windo] History search: '$searchQuery' ($($slice.Count) match(es), limit $hn)" -ForegroundColor Cyan
            foreach ($e in $slice) {
        ...
        if ($JsonOutput) { _emit_json "history" @{ mode = "tail"; entries = $slice; count = $slice.Count; exitCode = 0 }; return }
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "doctor") {
        ...
        if ($JsonOutput) {
            $ix = _integrity_status
            $lm = _read_last_meta
            $runnerLifecycle = _windo_runner_lifecycle_state
            $docExit = 0
        ...
        $runnerLifecycleDoc = _windo_runner_lifecycle_state
        $docExitT = 0
        ...
        Write-Host "  Dr. Run   : $($runnerLifecycleDoc.state)  (cleanup candidates $($runnerLifecycleDoc.cleanup.candidateCount))" -ForegroundColor $(if ($runnerLifecycleDoc.ready) { 'Green' } elseif ($runnerLifecycleDoc.state -eq 'repair') { 'Red' } else { 'Yellow' })
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "cleanup") {
        ...
        $runnerApply = $false
        $runnerOlderThanDays = 7
        $unknown = [System.Collections.ArrayList]@()
        for ($ci = 0; $ci -lt $cleanupRest.Count; $ci++) {
            $ca = [string]$cleanupRest[$ci]
            if ([string]::IsNullOrWhiteSpace($ca) -or $ca -eq '-w') { continue }
            if ($ca -eq '--runner-apply') { $runnerApply = $true; continue }
            if ($ca -eq '--runner-dry-run') { $runnerApply = $false; continue }
            if ($ca -in @('--older-than', '--older-than-days')) {
                if ($ci + 1 -lt $cleanupRest.Count) {
                    try { $runnerOlderThanDays = [int]$cleanupRest[$ci + 1] } catch { [void]$unknown.Add("$ca requires an integer") }
                    $ci++
                } else {
                    [void]$unknown.Add("$ca requires a value")
                }
                continue
            }
            if ($ca -like '--older-than=*') {
                try { $runnerOlderThanDays = [int]$ca.Substring(13) } catch { [void]$unknown.Add('--older-than requires an integer') }
                continue
            }
            [void]$unknown.Add($ca)
        }
        ...
        $runnerCleanup = _windo_runner_cleanup_execute -Apply:$runnerApply -OlderThanDays $runnerOlderThanDays
        if ($runnerApply) {
            Write-Host "[windo] Runner artifacts deleted: $($runnerCleanup.deletedCount) (skipped $($runnerCleanup.skippedCount))." -ForegroundColor $(if ($runnerCleanup.skippedCount -gt 0) { 'Yellow' } else { 'Green' })
        } else {
            Write-Host "[windo] Runner artifact cleanup is dry-run by default: $($runnerCleanup.candidateCount) candidate(s)." -ForegroundColor DarkGray
            Write-Host "  Apply with: windo cleanup --runner-apply --older-than $runnerOlderThanDays" -ForegroundColor DarkGray
            Write-Host "  Or inspect: windo runner cleanup --dry-run --older-than $runnerOlderThanDays" -ForegroundColor DarkGray
        }
        return
    }
...
function __windo_completion_specs {
    $specs = @{
        help = @('version','install-latest','source','trust','scan','net-scan','rdp','vnc','vault','sshx','crypto','explain','syntax','mesh','completion','output','motion','surface','integrate','control','signal','center','studio','edition','keybindings','recipes','venv','pyenv','python','pkg','package','installer','verbosity','container','wsl','launchpad','health','check','preflight','status','dashboard','integrity','verify','config','profile','modules','extras','ai','repair','run','--help','-h','-?','/?')
        ...
        source = @('--json')
        contract = @('doctor','--doctor','--json')
        explain = @('install-latest','source','contract','contract doctor','trust --online','recipes preview','recipes run','launchpad --tray','--json','--')
        ...
        backups = @('--json','--prune','--keep','--max-age-days','--force')
        cleanup = @('-w','--prune','--keep','--max-age-days','--force')
        log = @('-n','--tail','--json')
        history = @('search','-n','--contains','--regex','--since','--last-days','--activity','--json')
        ...
        upd = @('--force','--non-interactive','--timeout','--preserve-env')
        upgrade = @('--force','--non-interactive','--timeout','--preserve-env')
        do = @('--recipe','--dry-run')
        'up' = @('--force','--non-interactive','--timeout','--preserve-env')
    }
    $specs["recdo"] = @($specs["recipes"] + "--dry-run")
    $specs["up"] = $specs["install-latest"]
        return $specs
}
...
# WINDO profile.d loader for user-owned shell customization.
# Keep custom edits out of the managed WINDO block; files here survive every update.
try {
    $__windoProfileDirs = @(
        (Join-Path (Join-Path $HOME 'Documents') 'windo\profile.d'),
        (Join-Path (Join-Path $HOME '.pwsh_secure') 'profile.d')
    )
    foreach ($__windoProfileDir in $__windoProfileDirs) {
        if (-not (Test-Path -LiteralPath $__windoProfileDir)) { continue }
        foreach ($__windoProfileScript in @(Get-ChildItem -LiteralPath $__windoProfileDir -Filter '*.ps1' -File -ErrorAction SilentlyContinue | Sort-Object Name)) {
            try {
                $__tokens = $null
                $__errors = $null
                [void][System.Management.Automation.Language.Parser]::ParseFile($__windoProfileScript.FullName, [ref]$__tokens, [ref]$__errors)
                if ($__errors -and $__errors.Count -gt 0) {
                    Write-Warning ("WINDO profile.d skipped '" + $__windoProfileScript.FullName + "': " + $__errors[0].Message)
                    continue
                }
                . $__windoProfileScript.FullName
            } catch {
                Write-Warning ("WINDO profile.d failed '" + $__windoProfileScript.FullName + "': " + $_.Exception.Message)
            }
        }
    }
} catch {
    Write-Warning ("WINDO profile.d loader skipped: " + $_.Exception.Message)
}
'@
...
$block += "`r`n"
$newProfileText = $profileText.TrimEnd() + "`r`n`r`n" + $block
Test-WindoProfileSyntax -Content $newProfileText -Path ([string]$PROFILE) | Out-Null
