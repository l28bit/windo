...
$WindoBuiltinVerbs = @(
    ...
    'modules', 'recipes', 'prompt', 'extras', 'dev', 'session', 'status', 'ai', 'repair', 'scan', 'vault', 'sshx', 'crypto', 'venv', 'pyenv', 'python', 'pkg', 'package', 'installer', 'net-scan', 'rdp', 'vnc', 'container', 'wsl', 'run'
)
...
function Write-WindoEditionBanner {
    ...
    Write-Host ""
    Write-Host ("  WINDO {0}  V8.4  ::  {1}" -f $WindoVersion, $Phase.ToUpperInvariant()) -ForegroundColor White
...
function Invoke-WindoBundledUninstall {
    ...
    $TempUn = $null
    $ScriptPath = $null
        $handoffPlan = _windo_new_command_plan @("uninstall")
        $handoffSessionId = [Guid]::NewGuid().ToString("n")
    ...
    try {
        ...
        if ($KeepSnapshots) { $argList += '-KeepSnapshots' }
            $handoffEntry = @{
                ...
                Command = "windo uninstall"
                Elevation = "HANDOFF"
                Version = $WindoVersion
            }
            $handoffEntry = _windo_append_handoff_audit_fields $handoffEntry $handoffPlan.privilegeBoundary "WindoBundledUninstall" $handoffSessionId
        ...
        Write-Host "[windo] Starting elevated uninstall (UAC). Approve the prompt to remove tasks, profile blocks, and WINDO data." -ForegroundColor Yellow
            Write-Host "[windo] Uninstall handoff context: $($handoffPlan.privilegeBoundary)." -ForegroundColor Cyan
        try {
                $uninstallTimer = [Diagnostics.Stopwatch]::StartNew()
                $uninstallProcess = Start-Process -FilePath "powershell.exe" -Verb RunAs -ArgumentList $argList -Wait -PassThru -ErrorAction Stop
                $uninstallTimer.Stop()
                if (_windo_handoff_audit_enabled) {
                    $handoffEntry.DurationMs = [int]$uninstallTimer.Elapsed.TotalMilliseconds
                    $handoffEntry.ExitCode = if ($null -ne $uninstallProcess -and $uninstallProcess.PSObject.Properties.Name -contains "ExitCode") { [int]$uninstallProcess.ExitCode } else { 0 }
                    $handoffEntry.Elevation = "UAC"
                    $handoffEntry.Output = "[UAC handoff] uninstaller run requested."
                    _append_log $handoffEntry
                }
        } catch [System.ComponentModel.Win32Exception] {
            if ($_.Exception.NativeErrorCode -eq 1223) {
                    if (_windo_handoff_audit_enabled) {
                        $handoffEntry.DurationMs = 0
                        $handoffEntry.Elevation = "DECLINED"
                        $handoffEntry.ExitCode = 1223
                        $handoffEntry.Output = "[UAC declined] uninstall handoff was not approved."
                        _append_log $handoffEntry
                    }
...
function windo {
    ...
    $HelpRequested = $false
    $HelpTopic = $null
    $resolveTwoWordHelpTopic = {
        param(
            [System.Collections.IList]$Tokens,
            [int]$Index
        )
        if ($Index -lt 0 -or $Index -ge $Tokens.Count) { return @($null, 0) }
        $first = [string]$Tokens[$Index]
        if ($first -notlike '-*' -and $first -ne '/?' -and $first -ne '?' -and $first -ne '-?') {
            $candidate = $first
            $consumed = 1
            if (($Index + 1) -lt $Tokens.Count) {
                $second = [string]$Tokens[$Index + 1]
                if ($second -notlike '-*' -and $second -ne '/?' -and $second -ne '?' -and $second -ne '-?') {
                    $twoWord = ("$first $second").ToLowerInvariant().Trim()
                    if (
                        $twoWord -eq "install latest" -or
                        $twoWord -eq "self update" -or
                        $twoWord -eq "python venv"
                    ) {
                        $candidate = $twoWord
                        $consumed = 2
                    }
    ...
    if ($Command -and $Command.Count -gt 0) {
        ...
        if ($tx -eq "/?" -or $tx -eq "?") {
                ...
                if (($leading + 1) -lt $rawCommand.Count) {
                    $helpTopicResult = & $resolveTwoWordHelpTopic $rawCommand ($leading + 1)
                    if ($helpTopicResult[1] -gt 0) {
                        $HelpTopic = $helpTopicResult[0]
                        $leading += 1 + $helpTopicResult[1]
                        continue
                    }
                }
                $leading++
        ...
        if ($Command.Count -ge 1 -and ($Command[-1] -eq "/?" -or $Command[-1] -eq "?")) {
            $trimmed = @($Command[0..($Command.Count - 2)])
            $HelpRequested = $true
            if ([string]::IsNullOrWhiteSpace($HelpTopic)) {
                if ($trimmed.Count -gt 1 -and ($trimmed[0] -ieq 'help')) {
                    if ($trimmed.Count -ge 3 -and ([string]$trimmed[1]).ToLowerInvariant().Trim() -eq "install" -and [string]$trimmed[2].ToLowerInvariant().Trim() -eq "latest") {
                        $HelpTopic = "install latest"
                    } elseif ($trimmed.Count -ge 3 -and ([string]$trimmed[1]).ToLowerInvariant().Trim() -eq "self" -and [string]$trimmed[2].ToLowerInvariant().Trim() -eq "update") {
                        $HelpTopic = "self update"
                    } elseif ($trimmed.Count -ge 3 -and ([string]$trimmed[1]).ToLowerInvariant().Trim() -eq "python" -and [string]$trimmed[2].ToLowerInvariant().Trim() -eq "venv") {
                        $HelpTopic = "python venv"
                    } elseif ($trimmed.Count -ge 2) {
                        $headTwo = (([string]$trimmed[0]).ToLowerInvariant().Trim() + " " + ([string]$trimmed[1]).ToLowerInvariant().Trim())
                        if ($headTwo -eq "install latest" -or $headTwo -eq "self update" -or $headTwo -eq "python venv") {
                            $HelpTopic = $headTwo
                        } else {
                            $HelpTopic = [string]$trimmed[0]
                        }
                    } else {
                        $HelpTopic = [string]$trimmed[1]
                    }
                }
        ...
        if ($Command.Count -gt 0) {
            ...
            elseif ($firstToken -eq "pyenv") {
                if ($Command.Count -gt 1) { $Command = @("venv") + @($Command[1..($Command.Count - 1)]) } else { $Command = @("venv") }
            }
                ...
                if ($tx -eq '/?' -or (($tx -eq '--help' -or $tx -eq '-h' -or $tx -eq '-?') -and $isBuiltinHelpTarget)) {
                    ...
                    if ([string]::IsNullOrWhiteSpace($HelpTopic)) {
                        if ($hasCmd -and $cmd -eq 'help' -and ($i + 1) -lt $cl.Count) {
                            if (($i + 2) -lt $cl.Count -and ([string]$cl[$i + 1]).ToLowerInvariant().Trim() -eq "install" -and [string]$cl[$i + 2].ToLowerInvariant().Trim() -eq "latest") {
                                $HelpTopic = "install latest"
                            } elseif (($i + 2) -lt $cl.Count -and ([string]$cl[$i + 1]).ToLowerInvariant().Trim() -eq "self" -and [string]$cl[$i + 2].ToLowerInvariant().Trim() -eq "update") {
                                $HelpTopic = "self update"
                            } elseif (($i + 2) -lt $cl.Count -and ([string]$cl[$i + 1]).ToLowerInvariant().Trim() -eq "python" -and [string]$cl[$i + 2].ToLowerInvariant().Trim() -eq "venv") {
                                $HelpTopic = "python venv"
                            } else {
...
function _windo_parse_runner_result([string]$OutPath, [string]$RequestId, [string]$Command, [int]$FallbackDurationMs) {
    ...
    function _windo_resolve_output_policy {
        ...
        if (_windo_parse_bool_value $env:CI $false -and $mode -eq "legacy") { $mode = "compact" }
        $desc = switch ($mode) {
...
"@
    ...
    function _windo_new_command_plan([object[]]$Parts) {
        ...
        switch ($verb) {
            ...
            { $_ -in @("version", "config", "context", "profile", "roadmap", "preflight", "health", "check", "status", "doctor", "dashboard", "integrity", "verify", "session", "log", "history", "stats", "backups", "modules", "extras", "prompt", "ai", "help", "motion", "surface", "control", "signal", "center") } {
    ...
    function _windo_handoff_audit_enabled() {
        return _windo_parse_bool_value $env:WINDO_SESSION_AUDIT $false
    }

    function _windo_append_handoff_audit_fields([hashtable]$Entry, [string]$PrivilegeBoundaryContext, [string]$ElevationType, [string]$HandoffSessionId) {
        if (!(_windo_handoff_audit_enabled) -or $null -eq $Entry) { return $Entry }
        $proc = $null
        try { $proc = Get-Process -Id $PID -ErrorAction SilentlyContinue } catch {}

        $Entry.PrivilegeBoundaryContext = $PrivilegeBoundaryContext
        if (-not [string]::IsNullOrWhiteSpace($ElevationType)) { $Entry.ElevationType = $ElevationType }
        if (-not [string]::IsNullOrWhiteSpace($HandoffSessionId)) { $Entry.HandoffSessionId = $HandoffSessionId }
        $Entry.OriginalProcessUser = if ($env:USERNAME) { if ($env:USERDOMAIN) { "$env:USERDOMAIN\$env:USERNAME" } else { [string]$env:USERNAME } } else { "unknown" }
        $Entry.HandoffRequestedAt = (Get-Date).ToString("o")
        if ($null -ne $proc) {
            $Entry.CallingProcessId = [int]$proc.Id
            $Entry.CallingProcessName = [string]$proc.ProcessName
            try { if (-not [string]::IsNullOrWhiteSpace($proc.Path)) { $Entry.CallingProcessPath = [string]$proc.Path } } catch {}
        }
        return $Entry
    }
    ...
    function _windo_start_downloaded_installer {
        param(
            ...
            [string]$PrivilegeBoundaryContext = "network download, checksum verification, then elevated installer handoff",
            [string]$HandoffSessionId = ""
        )
        ...
        $argList = @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $ScriptPath)
        $ciMode = _windo_parse_bool_value $env:CI $false
        ...
        $handoffEntry = @{
            ...
            User = "$env:USERDOMAIN\$env:USERNAME"
            Host = $env:COMPUTERNAME
            Command = "$DisplayCommand installer handoff"
            Elevation = "HANDOFF"
            Version = $WindoVersion
        }
        $handoffEntry = _windo_append_handoff_audit_fields $handoffEntry $PrivilegeBoundaryContext "Installer" $HandoffSessionId
        ...
        if ($shouldElevate) {
            ...
            try {
                ...
                $handoffTimer.Stop()
                if (_windo_handoff_audit_enabled) {
                    $handoffEntry.DurationMs = [int]$handoffTimer.Elapsed.TotalMilliseconds
                    $handoffEntry.ExitCode = if ($null -ne $installerProcess -and $installerProcess.PSObject.Properties.Name -contains "ExitCode") { [int]$installerProcess.ExitCode } else { 0 }
                    $handoffEntry.Elevation = "UAC"
                    $handoffEntry.Output = "[UAC handoff] installer run requested."
                    _append_log $handoffEntry
                }
                ...
                if ($_.Exception.NativeErrorCode -eq 1223) {
                    ...
                    if (_windo_handoff_audit_enabled) {
                        ...
                        $handoffEntry.Elevation = "DECLINED"
                        $handoffEntry.ExitCode = 1223
                        $handoffEntry.Output = "[UAC declined] installer handoff was not approved."
                        _append_log $handoffEntry
                    }
                    return $false
                }
    ...
    function _windo_run_exodus_installer {
        param(
        ...
        $handoffPlan = _windo_new_command_plan @($DisplayCommand)
        $handoffSessionId = [Guid]::NewGuid().ToString("n")
        $ciMode = _windo_parse_bool_value $env:CI $false
        $nonInteractiveMode = _windo_parse_bool_value $env:WINDO_INSTALL_NONINTERACTIVE $false
        if (_windo_is_process_elevated) {
            Write-Host "[windo] ${DisplayCommand} handoff is blocked from elevated sessions." -ForegroundColor Yellow
            Write-Host "  Open a normal (non-elevated) PowerShell window and run:  windo $DisplayCommand" -ForegroundColor DarkGray
        ...
        try {
            ...
            Write-Host "[windo] ${DisplayCommand} download completed and checksum verified for ${branch}." -ForegroundColor Green
            $runNow = $false
            if ($ForceContinue -or $nonInteractiveMode -or $ciMode) {
                $runNow = $true
                if ($nonInteractiveMode -or $ciMode) {
                    Write-Host "[windo] Proceeding without confirmation for ${DisplayCommand} (automation/CI mode)." -ForegroundColor DarkGray
                }
                ...
                Write-Host "[windo] The ${DisplayCommand} payload is ready. You can review it before continuing: $TempInst" -ForegroundColor Cyan
                if (_windo_prompt_installer_handoff -NonInteractive:$NonInteractive -DisplayCommand $DisplayCommand -PromptText $env:SUDO_PROMPT -DefaultPrompt $defaultPrompt) {
                    $runNow = $true
                }
            }
            if (-not $runNow) {
                Write-Host "[windo] ${DisplayCommand} handoff was cancelled; temporary installer removed." -ForegroundColor DarkYellow
                Remove-Item -LiteralPath $TempInst -Force -ErrorAction SilentlyContinue
                _windo_set_exit 0
            ...
            Write-Host "[windo] Starting ${DisplayCommand} installer. When it finishes, reload profile: . `$PROFILE" -ForegroundColor Yellow
            $installerStarted = _windo_start_downloaded_installer -ScriptPath $TempInst -DisplayCommand $DisplayCommand -PrivilegeBoundaryContext $handoffPlan.privilegeBoundary -HandoffSessionId $handoffSessionId
    ...
    function _windo_run_genisis_installer {
        ...
        return _windo_run_exodus_installer -ForceContinue:$ForceContinue -NonInteractive:$NonInteractive -DisplayCommand:$DisplayCommand
    }
    ...
    function _append_log([hashtable]$entry) {
        ...
        $limit = 0
        if (Get-Command _windo_active_log_max_lines -CommandType Function -ErrorAction SilentlyContinue) {
            $limit = _windo_active_log_max_lines
        }
        if ($limit -gt 0) {
            $rotation = _windo_rotate_active_log_if_needed -MaxLines $limit
            if ($null -ne $rotation -and [bool]$rotation.rotated) {
                Write-Host "[windo] Active log rotated to keep history bounded: $($rotation.previousLineCount) -> $($rotation.maxLines) lines" -ForegroundColor DarkYellow
                Write-Host "[windo] Backup: $($rotation.backup)" -ForegroundColor DarkYellow
            } elseif ($null -ne $rotation -and -not [bool]$rotation.rotated -and $rotation.error) {
                Write-Host "[windo] Warning: active log retention rotation failed: $($rotation.error)" -ForegroundColor DarkGray
            }
        }
    }
    ...
    function _windo_prompt_installer_handoff {
        ...
        $ciMode = _windo_parse_bool_value $env:CI $false
        if ($NonInteractive -or $ciMode -or -not [Environment]::UserInteractive) { return $false }

        $prompt = _windo_normalize_handoff_prompt -PromptText $PromptText -DefaultPrompt $DefaultPrompt
        $commandMeaning = switch ($DisplayCommand) {
            "upgrade" { "upgrade" }
            "self-update" { "repair/update" }
            default { "install-latest" }
        }
        Write-Host "[windo] $commandMeaning handoff requires a short approval step before running elevation." -ForegroundColor Cyan
    ...
    function _windo_parse_positive_int {
        param(
            [string]$Raw,
            [int]$Default = 0,
            [string]$Name = "value"
    ...
    function _windo_audit_backups {
        param([string]$Path = $SecureDir)
        try {
            $bakFiles = @(
                Get-ChildItem -Path $Path -Filter "windo_history*.enc.bak" -File -ErrorAction SilentlyContinue |
        ...
        return @{ files = @($bakFiles); rows = @($rows); count = $bakFiles.Count }
    }

    function _windo_backups_to_prune {
        param([array]$Files, [int]$Keep = 0, [int]$MaxAgeDays = 0)
        $sorted = @($Files | Sort-Object LastWriteTime -Descending)
        if ($sorted.Count -eq 0) { return @() }
        $keepN = [int]$Keep
        if ($keepN -lt 0) { $keepN = 0 }
        $useMaxAge = $MaxAgeDays -gt 0
        if (-not $useMaxAge -and $keepN -le 0) { return @() }
        $cutoff = [datetime]::MinValue
        if ($useMaxAge) { $cutoff = (Get-Date).Date.AddDays(-$MaxAgeDays) }
        $remove = [System.Collections.ArrayList]@()
        for ($i = 0; $i -lt $sorted.Count; $i++) {
            $f = $sorted[$i]
            $drop = $false
            if ($keepN -gt 0 -and $i -ge $keepN) { $drop = $true }
            if (-not $drop -and $useMaxAge -and $f.LastWriteTime -lt $cutoff) { $drop = $true }
            if ($drop) { [void]$remove.Add($f) }
        }
        return @($remove)
    }

    function _windo_active_log_max_lines {
        $raw = $env:WINDO_AUDIT_LOG_MAX_LINES
    ...
    function _windo_rotate_active_log_if_needed {
        param([int]$MaxLines = 0)
        if ($MaxLines -le 0) { return @{ rotated = $false; previousLineCount = 0; maxLines = [int]$MaxLines } }
        if (!(Test-Path $LogFile)) { return @{ rotated = $false; previousLineCount = 0; maxLines = [int]$MaxLines } }
        $currentCount = _windo_log_line_count $LogFile
        if ($currentCount -le $MaxLines) { return @{ rotated = $false; previousLineCount = $currentCount; maxLines = [int]$MaxLines } }
        try {
            ...
            return @{
                rotated = $true
                backup = $backup
                previousLineCount = $currentCount
...
function _windo_draw_ascii_startup_frame {
    ...
    function _windo_resolve_motion_policy {
        ...
        $enabled = $false
        $ciMode = _windo_parse_bool_value $env:CI $false
        $noSpinner = _windo_parse_bool_value $env:WINDO_NO_SPINNER $false
        if ($mode -eq "on") { $enabled = $true }
        elseif ($mode -eq "auto") { $enabled = ($interactive -and -not $ciMode -and -not $noSpinner -and -not (_windo_parse_bool_value $reducedMotionRaw $false)) }
    ...
    function _windo_get_published_installer_sha256 {
        ...
        $releaseContext = _windo_release_context
        $resolvedRef = [string]$releaseContext.resolvedRef
        $sourceReleaseBranch = $releaseContext.branch
        $apiError = $null
        ...
        try {
            ...
            $manifestBranch = if ($null -ne $resolved -and $resolved.releaseBranch) { [string]$resolved.releaseBranch } else { $null }
            ...
            $metadataState = _windo_release_metadata_state -ResolvedRef $resolvedRef -ReleaseCommit $releaseCommit -ReleaseCommitRaw $releaseCommitRaw -ReleaseBranch $manifestBranch
            if (_is_sha256_hex $sha) {
                return [pscustomobject]@{
                    ...
                    sha256 = $sha
                    blobSha = $blobSha
                    releaseRef = $resolvedRef
                    releaseBranch = $manifestBranch
                    requestedRef = $releaseContext.requested
                    requestedType = $releaseContext.requestedType
                    sourceReleaseRef = $resolvedRef
        ...
        for ($attempt = 1; $attempt -le 3; $attempt++) {
            try {
                ...
                $rawMetadata = $null
                try { $rawMetadata = _windo_parse_release_metadata $rawContent } catch { }
                $releaseCommit = if ($null -ne $rawMetadata -and $rawMetadata.releaseCommit) { [string]$rawMetadata.releaseCommit } else { $null }
                $rawReleaseBranch = if ($null -ne $rawMetadata -and $rawMetadata.releaseBranch) { [string]$rawMetadata.releaseBranch } else { $null }
                $releaseCommitRaw = if ($null -ne $rawMetadata -and $rawMetadata.PSObject.Properties.Name -contains "releaseCommitRaw") { [string]$rawMetadata.releaseCommitRaw } else { $null }
                $releaseBranchRaw = if ($null -ne $rawMetadata -and $rawMetadata.PSObject.Properties.Name -contains "releaseBranchRaw") { [string]$rawMetadata.releaseBranchRaw } else { $null }
                $metadataState = _windo_release_metadata_state -ResolvedRef $resolvedRef -ReleaseCommit $releaseCommit -ReleaseCommitRaw $releaseCommitRaw -ReleaseBranch $rawReleaseBranch
                if (_is_sha256_hex $sha) {
                    return [pscustomobject]@{
                        status = "available"
                        source = "raw-fallback"
                        ...
                        blobSha = $null
                        releaseRef = $resolvedRef
                        releaseBranch = $rawReleaseBranch
                        requestedRef = $releaseContext.requested
                        requestedType = $releaseContext.requestedType
                        sourceReleaseRef = $resolvedRef
                        sourceReleaseBranch = $sourceReleaseBranch
                ...
                return [pscustomobject]@{
                    ...
                    sourceReleaseBranch = $sourceReleaseBranch
                    compatibilityMode = $metadataState.CompatibilityMode
                    compatibilityReason = if ($metadataState.Detail) { $metadataState.Detail } else { "published checksum was reachable but could not be normalized" }
                    releaseCommit = $releaseCommit
                    releaseCommitRaw = $releaseCommitRaw
                    releaseBranchRaw = $releaseBranchRaw
                    error = "published checksum was reachable but did not contain a valid SHA256"
                ...
                if (-not (_windo_is_retryable_web_error $_) -or $attempt -ge 3) {
                    ...
                    return [pscustomobject]@{
                        ...
                        blobSha = $null
                        releaseRef = $resolvedRef
                        releaseBranch = $sourceReleaseBranch
                        requestedRef = $releaseContext.requested
                        requestedType = $releaseContext.requestedType
                        sourceReleaseRef = $resolvedRef
                        sourceReleaseBranch = $sourceReleaseBranch
                        compatibilityMode = $false
                        compatibilityReason = $msg
                        releaseCommit = "unknown"
                        releaseCommitRaw = "unknown"
                        releaseBranchRaw = "unknown"
                        error = $msg
                    }
                }
                Start-Sleep -Milliseconds $script:_windo_retry_delays_ms[[Math]::Min($attempt - 1, 2)]
    ...
    function _windo_match_history_activity {
        param([string]$CommandText, [string]$Activity)
        if ([string]::IsNullOrWhiteSpace($Activity)) { return $false }
        if ([string]::IsNullOrWhiteSpace($CommandText)) { return $false }
        $ct = (" " + [string]$CommandText + " ").ToLowerInvariant()
        $activityNorm = [string]$Activity.Trim().ToLowerInvariant()
        if ($activityNorm -eq "install") {
            return ($ct -match '(?i)(\s|["\'])windo(?:\.exe)?\s+(install|install-latest)(\s|["'';]|$)')
        }
        if ($activityNorm -eq "upgrade") {
            return ($ct -match '(?i)(\s|["\'])windo(?:\.exe)?\s+upgrade(\s|["'';]|$)')
        }
        return $false
    }

    function _windo_filter_entries_by_activity {
        param($entries, [string]$Activity)
        if ([string]::IsNullOrWhiteSpace($Activity)) { return ,@($entries) }
        $out = [System.Collections.ArrayList]@()
        foreach ($entry in @($entries)) {
            $cmd = if ($entry -and $entry.PSObject.Properties.Name -contains 'Command') { [string]$entry.Command } else { "" }
            if (_windo_match_history_activity $cmd $Activity) { [void]$out.Add($entry) }
        }
        return ,@($out)
    }

    function _windo_filter_entries_by_command_contains {
        param($entries, [string]$Needle)
        if ([string]::IsNullOrWhiteSpace($Needle)) { return ,@($entries) }
        $needle = [string]$Needle
        $out = [System.Collections.ArrayList]@()
        foreach ($entry in @($entries)) {
            if ($entry -and $entry.PSObject.Properties.Name -contains 'Command') {
                $cmd = [string]$entry.Command
                if ($cmd.IndexOf($needle, [StringComparison]::InvariantCultureIgnoreCase) -ge 0) { [void]$out.Add($entry) }
            }
        }
        return ,@($out)
    }
    ...
    function _windo_write_edition_html([object]$Edition, [string]$OutputPath, [bool]$Open) {
        ...
        $null = $sb.AppendLine(("<div class='card'><div class='k'>Branch</div><div class='v'>{0}</div><div class='muted'>v{1}</div></div>" -f (_html_escape $Edition.branch), (_html_escape $Edition.windoVersion)))
...
"@
    ...
    function _windo_control_start_action([string]$ActionId) {
        ...
        $exitCode = 0
        if ($null -ne $process -and $process.PSObject.Properties.Name -contains "ExitCode") {
            if ($process.PSObject.Properties.Name -contains "HasExited" -and [bool]$process.HasExited) {
                $exitCode = [int]$process.ExitCode
            } elseif (-not ($process.PSObject.Properties.Name -contains "HasExited")) {
                $exitCode = [int]$process.ExitCode
            }
        }
        $metadata = @{
        ...
        if ($null -ne $process -and $process.PSObject.Properties.Name -contains "Id") { $metadata["processId"] = [int]$process.Id }
        if ($process) { $metadata["status"] = if ([bool]$process.HasExited) { if ($exitCode -ne 0) { "completed" } else { "completed" } } else { "running" } }
        else { $metadata["status"] = "running" }
    ...
    function _windo_help_topics {
        return @(
            ...
            [pscustomobject]@{
                ...
                Aliases     = @("upgrade")
                Category    = "Core"
                Summary     = "Download and run latest installer from the tracked release branch."
            ...
            [pscustomobject]@{
                ...
                Description = "Uses the bundled local uninstaller when present, otherwise downloads it from the tracked release branch, then starts an elevated uninstall."
        ...
        [pscustomobject]@{
            ...
            [pscustomobject]@{
                ...
                Examples    = @("windo control", "windo control actions", "windo control preview surface-prime", "windo control queue surface-prime", "windo control run diagnostics-snapshot", "windo control run log-bundle-open", "windo control execute-next", "windo control execute <id>")
            },
            [pscustomobject]@{
                Name        = "signal"
                Category    = "Inspection"
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo backups [--json]", "windo backups --prune --keep N --force", "windo backups --prune --max-age-days N --force", "windo backups --prune --keep N --max-age-days M --force")
                Description = "Enumerates and optionally prunes windo_history*.enc.bak files."
                Notes       = "Prune requires --force. Use --keep for oldest-file retention and/or --max-age-days for time-based retention."
                Examples    = @("windo backups", "windo backups --json", "windo backups --prune --keep 5 --force", "windo backups --prune --max-age-days 30 --force")
            },
            [pscustomobject]@{
                Name        = "theme"
                Category    = "Configuration"
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo doctor [--json]", "windo health [--json]")
                ...
                Examples    = @("windo doctor", "windo doctor --json", "windo health", "windo health --json")
            },
            [pscustomobject]@{
                Name        = "integrity"
                Category    = "Inspection"
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo history [-n N]", "windo history --since YYYY-MM-DD", "windo history --last-days N", "windo history --activity install|upgrade", "windo history --contains <text>")
                Description = "Shows compact summaries for last N entries."
                Notes       = "Defaults to 50 when -n is omitted. --since and --last-days are mutually exclusive. --activity accepts install or upgrade."
                Examples    = @("windo history", "windo history -n 20", "windo history --activity upgrade --last-days 14", "windo history --contains upgrade --json")
            },
            [pscustomobject]@{
                Name        = "report"
                Category    = "Reporting"
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo preflight [--json]", "windo check [--json]")
                ...
                Examples    = @("windo preflight", "windo preflight --json", "windo check", "windo check --json")
            },
            [pscustomobject]@{
                Name        = "launchpad"
                Category    = "Reporting"
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo cleanup [-w]", "windo cleanup --prune --keep N --force", "windo cleanup --prune --max-age-days N --force", "windo cleanup --prune --keep N --max-age-days M --force")
                Description = "Creates windo_history.<timestamp>.enc.bak and clears log. Optional prune applies retention rules immediately after backup."
                Notes       = "`-w` is accepted for compatibility and ignored."
                Examples    = @("windo cleanup", "windo cleanup -w", "windo cleanup --prune --keep 5 --force")
            },
            [pscustomobject]@{
                Name        = "modules"
                Category    = "Shell Experience"
            ...
            [pscustomobject]@{
                ...
                Description = "Downloads the published extras index from GitHub on the tracked release branch and searches entries. Fetch downloads artifacts only from a non-elevated shell and verifies SHA256 when published."
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo session [--json]", "windo status [--json]")
                ...
                Examples    = @("windo session", "windo session --json", "windo status", "windo status --json")
            },
            [pscustomobject]@{
                Name        = "ai"
                Category    = "Shell Experience"
            ...
            [pscustomobject]@{
                ...
                Notes       = "For a full profile refresh from the tracked release branch, run 'windo install-latest' from a non-elevated shell after repair."
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "help") {
        ...
        if ($Command.Count -ge 2) {
            if ($Command.Count -ge 3 -and ([string]$Command[1]).ToLowerInvariant().Trim() -eq "install" -and [string]$Command[2].ToLowerInvariant().Trim() -eq "latest") {
                $topic = "install latest"
            } elseif ($Command.Count -ge 3 -and ([string]$Command[1]).ToLowerInvariant().Trim() -eq "self" -and [string]$Command[2].ToLowerInvariant().Trim() -eq "update") {
                $topic = "self update"
            } elseif ($Command.Count -ge 3 -and ([string]$Command[1]).ToLowerInvariant().Trim() -eq "python" -and [string]$Command[2].ToLowerInvariant().Trim() -eq "venv") {
                $topic = "python venv"
            } else {
                $topic = [string]$Command[1]
            }
        }
        _windo_show_help $topic
        return
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "config") {
        ...
        $rows = [System.Collections.ArrayList]@(
            ...
            [pscustomobject]@{ name = "WINDO_SKIP_INSTALLER_SHA256"; environmentValue = $(if ($env:WINDO_SKIP_INSTALLER_SHA256) { [string]$env:WINDO_SKIP_INSTALLER_SHA256 } else { $null }); effectiveNote = $(if ($env:WINDO_SKIP_INSTALLER_SHA256) { "bootstrap/upgrade checksum check skipped" } else { "checksum enforced when published for the tracked release branch" }) }
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "backups") {
        ...
        while ($bi -lt $Command.Count) {
            ...
            if ($ba -eq "--keep" -and $bi + 1 -lt $Command.Count) {
                try {
                    $kp = _windo_parse_positive_int ([string]$Command[$bi + 1]) 0 "keep"
                    $keepN = $kp
                    $bi += 2
                    continue
                } catch {
                    Write-Host "[windo] backups: --keep requires a positive integer" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
            }
            if ($ba -like "--keep=*") {
                $rawKeep = ($ba -replace '^--keep=', '')
                try {
                    $keepN = _windo_parse_positive_int $rawKeep 0 "keep"
                    $bi++
                    continue
                } catch {
                    Write-Host "[windo] backups: --keep requires a positive integer" -ForegroundColor Red
            ...
            if ($ba -eq "--max-age-days" -and $bi + 1 -lt $Command.Count) {
                try {
                    $mad = _windo_parse_positive_int ([string]$Command[$bi + 1]) 0 "max age"
                    if ($mad -lt 1) { throw "invalid age" }
                    $maxAgeDays = $mad
                    $bi += 2
                    continue
                } catch {
                    Write-Host "[windo] backups: --max-age-days requires a positive integer" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
            }
            if ($ba -like "--max-age-days=*") {
                $rawAge = ($ba -replace '^--max-age-days=', '')
                try {
                    $mad = _windo_parse_positive_int $rawAge 0 "max age"
                    if ($mad -lt 1) { throw "invalid age" }
                    $maxAgeDays = $mad
                    $bi++
                    continue
            ...
            if ($ba -eq "--json") { $bi++; continue }
            Write-Host "[windo] backups: unknown argument: $ba" -ForegroundColor Yellow
            _windo_set_exit 2
            return
        }

        $backupState = _windo_audit_backups $SecureDir
        $bakFiles = @($backupState.files)
        if ($doPrune) {
            if (($null -eq $keepN -or [int]$keepN -lt 1) -and ($null -eq $maxAgeDays -or [int]$maxAgeDays -lt 1)) {
                if ($JsonOutput) {
                    _emit_json "backups" @{
                        error = "--prune requires --keep N and/or --max-age-days N"
                        exitCode = 2
                        backups = @($backupState.rows)
                    }
                } else {
                    Write-Host "[windo] backups: --prune requires --keep N and/or --max-age-days N" -ForegroundColor Red
                }
                _windo_set_exit 2
                return
            }

            if ($null -ne $keepN -and [int]$keepN -lt 1) {
                if ($JsonOutput) {
                    _emit_json "backups" @{ error = "--keep must be at least 1"; exitCode = 2; backups = @($backupState.rows); keep = [int]$keepN }
                } else {
                    Write-Host "[windo] backups: --keep must be at least 1" -ForegroundColor Red
                }
                _windo_set_exit 2
                return
            }
            if ($null -ne $maxAgeDays -and [int]$maxAgeDays -lt 1) {
                if ($JsonOutput) {
                    _emit_json "backups" @{ error = "--max-age-days must be at least 1"; exitCode = 2; backups = @($backupState.rows); maxAgeDays = [int]$maxAgeDays }
                } else {
                    Write-Host "[windo] backups: --max-age-days must be at least 1" -ForegroundColor Red
                }
                _windo_set_exit 2
                return
            }

            $targets = @(_windo_backups_to_prune -Files $bakFiles -Keep $keepN -MaxAgeDays $maxAgeDays)
            if ($targets.Count -eq 0) {
                if ($JsonOutput) {
                    _emit_json "backups" @{
                        backups = @($backupState.rows)
                        prunedFiles = @()
                        keep = $(if ($null -ne $keepN) { [int]$keepN } else { $null })
                        maxAgeDays = $(if ($null -ne $maxAgeDays) { [int]$maxAgeDays } else { $null })
                        exitCode = 0
                        message = "no backups matched prune policy"
                    }
                } else {
                    Write-Host "[windo] backups: no files match prune policy." -ForegroundColor DarkGray
                }
                _windo_set_exit 0
                return
            }
            if (-not $forcePrune) {
                if ($JsonOutput) {
                    $names = [System.Collections.ArrayList]@()
                    foreach ($f in $targets) { [void]$names.Add($f.Name) }
                    _emit_json "backups" @{
                        error = "prune requires --force"
                        exitCode = 2
                        backups = @($backupState.rows)
                        keep = $(if ($null -ne $keepN) { [int]$keepN } else { $null })
                        maxAgeDays = $(if ($null -ne $maxAgeDays) { [int]$maxAgeDays } else { $null })
                        wouldRemove = @($names)
                        wouldRemoveCount = $targets.Count
                    }
                } else {
                    Write-Host "[windo] backups: prune would remove $($targets.Count) file(s); add --force to confirm." -ForegroundColor Yellow
                }
                _windo_set_exit 2
                return
            }
            ...
            foreach ($f in $targets) {
                try {
                    ...
                    if ($JsonOutput) {
                        _emit_json "backups" @{ error = "failed to remove $($f.Name): $($_.Exception.Message)"; exitCode = 2; backups = @($backupState.rows) }
                    } else {
            ...
            $rowsAfter = @(_windo_audit_backups $SecureDir).rows
            if ($JsonOutput) {
                _emit_json "backups" @{
                    backups = @($rowsAfter)
                    prunedFiles = @($removed)
                ...
                Write-Host "  Kept      : $($rowsAfter.Count) file(s)" -ForegroundColor DarkGray
                if ($null -ne $keepN) { Write-Host "  Policy --keep: $keepN" -ForegroundColor DarkGray }
                if ($null -ne $maxAgeDays) { Write-Host "  Policy --max-age-days: $maxAgeDays" -ForegroundColor DarkGray }
        ...
        if ($JsonOutput) {
            _emit_json "backups" @{ backups = @($backupState.rows); backupCount = $backupState.count; exitCode = 0; keep = $null; maxAgeDays = $null }
            _windo_set_exit 0
            return
        }
        ...
        if ($bakFiles.Count -eq 0) {
            ...
            Write-Host "  Or by age:  windo backups --prune --max-age-days 30 --force" -ForegroundColor DarkGray
        }
        _windo_set_exit 0
        return
    }
...
"@
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "repair") {
        ...
        $pol = $res.policy
        $hints = @(
            "Open a normal (non-elevated) terminal or run:  . `$PROFILE  - so your profile block matches prefs."
            "From a normal (non-elevated) window:  windo install-latest  - refreshes the embedded profile from the tracked release branch when you are behind."
        )
        if ($JsonOutput) {
            _emit_json "repair" @{
    ...
    if ($Command.Count -ge 1 -and ($Command[0] -eq "upgrade" -or $Command[0] -eq "install-latest")) {
        ...
        _windo_run_exodus_installer -ForceContinue:$forceInst -NonInteractive:$NonInteractive -DisplayCommand:$requestedUpdateCommand
        return
    }
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "history") {
        ...
        while ($hi -lt $Command.Count) {
            $ha = [string]$Command[$hi]
            if ($ha -eq "-n" -and $hi + 1 -lt $Command.Count) {
                try {
                    $hn = _windo_parse_positive_int ([string]$Command[$hi + 1]) 50 "n"
                } catch {
                    Write-Host "[windo] history: -n requires a positive integer" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
                $hi += 2
            ...
            if ($ha -eq "--since") {
                if ($hi + 1 -ge $Command.Count) {
                    Write-Host "[windo] history: --since requires YYYY-MM-DD" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
                $sinceStr = [string]$Command[$hi + 1]
                $hi += 2
                continue
            }
            if ($ha -like "--since=*") {
                $sinceStr = $ha.Substring(8).Trim()
                $hi++
                continue
            }
            if ($ha -eq "--last-days") {
                if ($hi + 1 -ge $Command.Count) {
                    Write-Host "[windo] history: --last-days requires a positive integer" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
                try {
                    $lastDaysN = _windo_parse_positive_int ([string]$Command[$hi + 1]) 0 "last-days"
                } catch {
                    Write-Host "[windo] history: invalid --last-days (use a positive integer, e.g. 7)" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
                $lastDaysSeen = $true
            ...
            if ($ha -like "--last-days=*") {
                try {
                    $lastDaysN = _windo_parse_positive_int ($ha.Substring(12).Trim()) 0 "last-days"
                    $lastDaysSeen = $true
                } catch {
                    Write-Host "[windo] history: invalid --last-days (use a positive integer, e.g. 7)" -ForegroundColor Red
                    _windo_set_exit 2
            ...
            if ($ha -eq "--activity") {
                if ($hi + 1 -ge $Command.Count) {
                    Write-Host "[windo] history: --activity requires install|upgrade" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
                $activity = [string]$Command[$hi + 1].Trim().ToLowerInvariant()
                if ($activity -ne "install" -and $activity -ne "upgrade") {
                    Write-Host "[windo] history: --activity supports install|upgrade" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
                $hi += 2
            ...
            if ($ha -like "--activity=*") {
                $activity = $ha.Substring(11).Trim().ToLowerInvariant()
                if ($activity -ne "install" -and $activity -ne "upgrade") {
                    Write-Host "[windo] history: --activity supports install|upgrade" -ForegroundColor Red
                    _windo_set_exit 2
                    return
            ...
            if ($ha -eq "--contains") {
                if ($hi + 1 -ge $Command.Count) {
                    Write-Host "[windo] history: --contains requires a value" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
                $contains = [string]$Command[$hi + 1]
                $hi += 2
                continue
            }
            if ($ha -like "--contains=*") {
                $contains = $ha.Substring(11)
                $hi++
                continue
            }
            if ($ha -eq "--json") { $hi++; continue }
            if ($ha -eq "-h" -or $ha -eq "--help") {
                Write-Host "[windo] Usage: windo history [-n N] [--since YYYY-MM-DD|--last-days N] [--activity install|upgrade] [--contains <text>] [--json]" -ForegroundColor Yellow
                _windo_set_exit 0
                return
            }
            Write-Host "[windo] history: unknown argument: $ha" -ForegroundColor Yellow
            _windo_set_exit 2
            return
        }
        if (Test-Path $LogFile) {
        ...
        if ($sinceStr) {
            try {
                ...
                Write-Host "[windo] history: invalid --since (use YYYY-MM-DD)" -ForegroundColor Red
                _windo_set_exit 2
                return
            }
        }
        if ($lastDaysSeen -and $null -ne $lastDaysN -and [int]$lastDaysN -le 0) {
            Write-Host "[windo] history: --last-days must be a positive integer" -ForegroundColor Red
            _windo_set_exit 2
            return
        }
        if ($null -ne $sinceDt -and $lastDaysSeen) {
            Write-Host "[windo] history: use only one of --since or --last-days" -ForegroundColor Yellow
            _windo_set_exit 2
            return
        }
        $all = @(_parse_log_entries -EmitWarnings $false)
        $filtered = @(_windo_filter_entries_by_time $all $sinceDt $lastDaysN)
        $filtered = @(_windo_filter_entries_by_activity $filtered $activity)
        $filtered = @(_windo_filter_entries_by_command_contains $filtered $contains)
        $slice = @($filtered | Select-Object -Last $hn)
        if ($JsonOutput) {
            _emit_json "history" @{
                entries = $slice
                count = $slice.Count
                ...
                filters = @{
                    take = [int]$hn
                    since = $(if ($sinceStr) { $sinceStr } else { $null })
                    lastDays = $(if ($lastDaysSeen) { [int]$lastDaysN } else { $null })
                    activity = $(if ($activity) { [string]$activity } else { $null })
                    contains = $(if ($contains) { [string]$contains } else { $null })
                }
                exitCode = 0
            }
            _windo_set_exit 0
        ...
        if ($sinceStr) { Write-Host "  Filter --since : $sinceStr" -ForegroundColor DarkGray }
        if ($lastDaysSeen -and $null -ne $lastDaysN) { Write-Host "  Filter --last-days : $lastDaysN" -ForegroundColor DarkGray }
        if ($activity) { Write-Host "  Filter --activity : $activity" -ForegroundColor DarkGray }
        if ($contains) { Write-Host "  Filter --contains : $contains" -ForegroundColor DarkGray }
        Write-Host "  Total matched : $($filtered.Count)" -ForegroundColor DarkGray
        Write-Host "  Showing last $($slice.Count)" -ForegroundColor DarkGray
        foreach ($e in $slice) {
        ...
        try {
            try {
                ...
                if (_windo_prompt_self_update_installer -NonInteractive:$NonInteractive) {
                    ...
                    $repairStarted = [bool](_windo_run_exodus_installer -ForceContinue:$true -DisplayCommand "self-update")
                    if ($repairStarted) {
                ...
                if ($NonInteractive -or (_windo_parse_bool_value $env:CI $false) -or -not [Environment]::UserInteractive) {
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "cleanup") {
        ...
        $cleanupForcePrune = $false
        $ci = 1
        while ($ci -lt $Command.Count) {
            $tk = [string]$Command[$ci]
            if ($tk -eq "-w") { $ci++; continue }
            if ($tk -eq "--prune") { $doPrune = $true; $ci++; continue }
            if ($tk -eq "--force") { $cleanupForcePrune = $true; $ci++; continue }
            if ($tk -eq "--keep" -and $ci + 1 -lt $Command.Count) {
                try {
                    $cleanupKeep = _windo_parse_positive_int ([string]$Command[$ci + 1]) 0 "keep"
                    $ci += 2
                    continue
                } catch {
                    Write-Host "[windo] cleanup: --keep requires a positive integer" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
            }
            if ($tk -like "--keep=*") {
                try {
                    $cleanupKeep = _windo_parse_positive_int ($tk -replace '^--keep=', '') 0 "keep"
                    $ci++
                    continue
                } catch {
                    Write-Host "[windo] cleanup: --keep requires a positive integer" -ForegroundColor Red
            ...
            if ($tk -eq "--max-age-days" -and $ci + 1 -lt $Command.Count) {
                try {
                    $cleanupMaxAgeDays = _windo_parse_positive_int ([string]$Command[$ci + 1]) 0 "max age"
                    if ($cleanupMaxAgeDays -lt 1) { throw "invalid age" }
                    $ci += 2
                    continue
                } catch {
                    Write-Host "[windo] cleanup: --max-age-days requires a positive integer" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
            }
            if ($tk -like "--max-age-days=*") {
                try {
                    $cleanupMaxAgeDays = _windo_parse_positive_int ($tk -replace '^--max-age-days=', '') 0 "max age"
                    if ($cleanupMaxAgeDays -lt 1) { throw "invalid age" }
                    $ci++
                    continue
                } catch {
            ...
            if ($tk -eq "--json") { $ci++; continue }
            Write-Host "[windo] cleanup: unknown argument(s): $tk" -ForegroundColor Yellow
            _windo_set_exit 2
            return
        }
        $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
        ...
        $pruned = [System.Collections.ArrayList]@()
        if ($doPrune) {
            if (($null -eq $cleanupKeep -or [int]$cleanupKeep -lt 1) -and ($null -eq $cleanupMaxAgeDays -or [int]$cleanupMaxAgeDays -lt 1)) {
                Write-Host "[windo] cleanup: --prune requires --keep N and/or --max-age-days N" -ForegroundColor Red
                _windo_set_exit 2
                return
            }
            if (-not $cleanupForcePrune) {
                Write-Host "[windo] cleanup: --prune requires --force to remove backup files" -ForegroundColor Yellow
                _windo_set_exit 2
                return
            }
            $beforeCleanup = _windo_audit_backups $SecureDir
            $targets = @(_windo_backups_to_prune -Files $beforeCleanup.files -Keep $cleanupKeep -MaxAgeDays $cleanupMaxAgeDays)
            foreach ($f in $targets) {
                try {
                    Remove-Item -LiteralPath $f.FullName -Force -ErrorAction Stop
                    [void]$pruned.Add($f.Name)
                } catch {
                    Write-Host "[windo] cleanup: failed to remove $($f.FullName)" -ForegroundColor Red
                    _windo_set_exit 2
                    return
                }
            }
        ...
        if ($JsonOutput) {
            $afterCleanup = _windo_audit_backups $SecureDir
            if ($doPrune) {
                _emit_json "cleanup" @{
                    backedUp = $backedUp
                    backup = $copied
                    prunedFiles = @($pruned)
                    keep = $(if ($null -ne $cleanupKeep) { [int]$cleanupKeep } else { $null })
                    maxAgeDays = $(if ($null -ne $cleanupMaxAgeDays) { [int]$cleanupMaxAgeDays } else { $null })
                    backups = @($afterCleanup.rows)
                    backupCount = $afterCleanup.count
                    exitCode = 0
                }
            } else {
                _emit_json "cleanup" @{ backedUp = $backedUp; backup = $copied; backups = @($afterCleanup.rows); backupCount = $afterCleanup.count; exitCode = 0 }
            }
        } else {
            if ($doPrune) {
                Write-Host "[windo] Cleanup complete and backup retention applied." -ForegroundColor Cyan
                if ($backedUp) { Write-Host "  Backup: $backup" -ForegroundColor DarkGray }
                if ($pruned.Count -gt 0) { Write-Host "  Removed backups: $($pruned.Count)" -ForegroundColor DarkYellow }
            } else {
                Write-Host "[windo] cleanup complete." -ForegroundColor Cyan
            }
        }
        return
    }
...
function __windo_completion_specs {
    $specs = @{
        ...
        log = @('-n','--tail','--json')
        history = @('-n','--since','--last-days','--activity','--contains','--json')
