...
## What's new in v3.6.3 Special Edition
...
- **Syntax Truth pass** — `windo /?` and global help flags are handled after helper load, and tab completion now knows WINDO command-specific syntax.
...
Full list: [`CHANGELOG.md`](CHANGELOG.md). Release copy: [`docs/releases/RELEASE_NOTES_v3.6.3.md`](docs/releases/RELEASE_NOTES_v3.6.3.md).
...
| `windo roadmap [--json]` | **v3.4.0+** Show the release train from Quiet Shell through V5. |
...
| `windo syntax [query] [--json]` | **v3.6.0+** Read-only intent-to-command planner with preview commands, risk notes, and aliases. |
...
| `windo recipes [list] \| show \| preview \| run` / `windo run --recipe <name>` | **v3.2.0+** Built-in elevated **recipe** templates (bundled data, not arbitrary script). **v3.6.0+** adds first-class preview and recipe dry-run payloads. **v3.2.6+** includes read-only **Ollama** peeks (**`ollama-list`**, **`ollama-ps`**, **`ollama-version`**) when **`ollama.exe`** is on PATH. |
...
**Shortcut:** **`windo repair`** (same as **`windo repair all`**) runs that safe-reset and prints reminders to **`. $PROFILE`** and run **`windo install-latest`** from a normal shell when your installed profile lags **Genisis**.
