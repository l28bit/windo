<# =====================================================================
WINDO v3.6.3 Special Edition Installer
...
function __windo_completion_specs {
    @{
        help = @('version','install-latest','trust','explain','syntax','completion','keybindings','recipes','launchpad','preflight','dashboard','integrity','verify','config','profile','modules','extras','ai','repair')
        completion = @('status','native-first','hybrid','windo','off','reset','--json')
        trust = @('--online','--offline','--json')
        explain = @('install-latest','trust --online','recipes preview','recipes run','launchpad --tray','--json','--')
