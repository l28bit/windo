...
"@
    ...
    function _windo_roadmap_releases {
        return @(
            ...
            [pscustomobject]@{
                version = "8.5.1"
                codename = "Profile Parse Guard"
                theme = "Make installer profile refreshes fail closed when generated PowerShell would not parse."
                focus = @("generated profile parse validation", "windo contract", "contract doctor", "history search", "source contract enrichment", "edition-aware branding")
                status = "in-progress"
                operatorValue = "Operators can update WINDO without a malformed generated profile replacing a working profile."
            }
        )
    }
