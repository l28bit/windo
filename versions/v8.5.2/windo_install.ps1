...
"@
    ...
    function _windo_roadmap_releases {
        return @(
            ...
            [pscustomobject]@{
                version = "8.5.2"
                codename = "Profile Reliability Contract"
                theme = "Make bootstrap and profile refreshes fail closed without touching user-owned profile content."
                focus = @("strict-mode-safe bootstrap web errors", "managed profile block v2", "profile.d customization loader", "generated profile parse validation", "profile write backup")
                status = "in-progress"
                operatorValue = "Operators can update WINDO without network fallback bugs or malformed generated blocks damaging their shell profile."
            }
        )
    }
