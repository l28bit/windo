...
function windo {
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "config") {
        ...
        if ($JsonOutput) {
            _emit_json "config" @{ settings = @($rows); secureDir = $SecureDir; exitCode = 0 }
            _windo_set_exit 0
            return
        }
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "theme") {
        ...
        if ($sub -in @('classic', 'modern', 'auto')) {
            try {
                ...
                ($prefObj | ConvertTo-Json) | Set-Content -Path $PrefsFile -Encoding UTF8
            } catch {
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "doctor") {
        ...
        Write-Host "    WINDO_MAX_COMMAND_CHARS, WINDO_SKIP_INSTALLER_SHA256, WINDO_JSON_ENVELOPE  (see README / SECURITY)" -ForegroundColor DarkGray
