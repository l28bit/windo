...
- **Release snapshots** — maintainers can run [`tools/Sync-VersionSnapshot.ps1`](tools/Sync-VersionSnapshot.ps1) to populate **`versions/vX.Y.Z/`** with the installer, checksum, **`README`**, **`SECURITY`**, **`CHANGELOG`**, **`docs/`**, and **`extras/`** (see [`docs/build.md`](docs/build.md)).
- **v3.2.2 highlights** (still in tree): **`windo export --json`**, **`help`** / **`export`** schema tables, runner **`try`/`finally`** fix, and expanded PSScriptAnalyzer coverage.
