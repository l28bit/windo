...
- **Framework wave map** — [`docs/framework-wave.md`](docs/framework-wave.md) records the completed composable-shell companion plan (modules, recipes, prompt, extras, dev, session, keybindings doctor) with links to operator and JSON docs—so the shipped checklist lives **in the repo**, not only in external planning notes.
...
**Optional shell companion layer** (modules, recipes, prompt bridge, curated extras—**v3.2.0+**): see [`docs/framework-wave.md`](docs/framework-wave.md) for how these features map to the shipped plan and trust model.
