...
"@
    ...
    function _windo_roadmap_releases {
        return @(
            ...
            [pscustomobject]@{
                version = "8.5.0"
                codename = "Contract Posture"
                theme = "Make release contract visibility actionable and enrich audit inspection without leaving the shell."
                focus = @("windo contract", "contract doctor", "history search", "source contract enrichment", "edition-aware branding")
                status = "in-progress"
                operatorValue = "Operators can run one contract posture check and search encrypted audit history by command text before exporting or reporting."
            }
        )
    }
