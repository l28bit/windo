<# =====================================================================
WINDO v5.2.0 Limited Edition Installer
...
"@
    ...
    function _windo_help_topics {
        return @(
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo center [status] [--json]", "windo center open|tray|panel", "windo center actions|signal|history", "windo center preview|run|queue <action-id>", "windo center execute-next|next", "windo center execute <request-id>")
                ...
                Examples    = @("windo center", "windo center panel", "windo center actions", "windo center preview surface-prime", "windo center queue surface-prime", "windo center execute-next", "windo center signal")
            },
            [pscustomobject]@{
                Name        = "edition"
                Category    = "Shell Experience"
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "surface") {
        ...
        if ($JsonOutput) { _emit_json "surface" @{ subcommand = "status"; surface = $state; exitCode = $exit } }
            ...
            Write-Host "  Open         : windo surface panel | windo launchpad --tray" -ForegroundColor DarkGray
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "center") {
        ...
        if ($sub -notin @("status", "")) {
            if ($JsonOutput) { _emit_json "center" @{ error = "expected status | open | tray | panel | actions | preview | run | queue | execute-next | next | execute | history | signal"; exitCode = 2 } } else { Write-Host "[windo] center: expected status | open | tray | panel | actions | preview | run | queue | execute-next | next | execute | history | signal" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        $center = _windo_center_state
...
function __windo_completion_specs {
    @{
        ...
        control = @('status','prime','actions','preview','queue','run','execute-next','next','execute','inspect','cancel','history','pulse','demo','clear','surface-status','surface-prime','surface-panel','launchpad-tray','workbench-html','edition-open','motion-pulse','profile-doctor','trust-online','preflight','install-latest','--json')
        signal = @('status','timeline','last','export','open','html','--html','--open','--output','--json')
        center = @('status','open','tray','panel','surface','actions','preview','run','queue','execute-next','next','execute','history','signal','surface-status','surface-prime','surface-panel','launchpad-tray','workbench-html','edition-open','motion-pulse','profile-doctor','trust-online','preflight','install-latest','--json')
