...
| `center` | 0, 2, 3 | **v5.0.0+ / v5.2.0+** status/open/tray/panel/actions/preview/run/queue/execute/history/signal success, bad subcommand/action, or attention/native surface state |
...
| Field | Type | Description |
|--------|------|-------------|
| `subcommand` | string | `status`, `open`, `tray`, `panel`, `actions`, `preview`, `run`, `queue`, `execute-next`, `next`, `execute`, `history`, or `signal`. |
