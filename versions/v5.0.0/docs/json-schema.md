...
| `command` | string | Logical command name for the envelope (e.g. `doctor`, `integrity`, `config`, `session`, `dashboard`, `preflight`, `launchpad`, `keybindings`, `completion`, `roadmap`, `source`, `syntax`, `mesh`, `repair`, `modules`, `extras`, `recipes`, `ai`, `backups`, `version`, `verify`, `log`, `stats`, `history`, `last`, `context`, `trace`, `profile`, `motion`, `surface`, `control`, `signal`, `center`; bundle-related commands when exporting, etc.) |
...
| `surface` | 0, 2, 3 | **v4.2.0+ / v4.6.0+** status/prime/pulse/doctor/repair/open success, write/repair failure, or readiness issues |
...
| `center` | 0, 2, 3 | **v5.0.0+** status/open/tray/run/queue/history success, bad subcommand/action, or attention state |
...
| Field | Type | Description |
|--------|------|-------------|
| `subcommand` | string | `status`, `prime`, `pulse`, `doctor`, `repair`, or `open`. |
...
| Field | Type | Description |
|--------|------|-------------|
| `subcommand` | string | `status`, `prime`, `actions`, `queue`, `run`, `execute-next`, `inspect`, `cancel`, `history`, `pulse`, or `clear`. |
...
| `result` | object | Present for `queue`, `run`, `execute-next`, or `cancel`; includes action/request status and paths where applicable. |
...
| Field | Type | Description |
|--------|------|-------------|
| `subcommand` | string | `status`, `timeline`, `last`, or `export`. |
...
| `htmlPath` | string | Present for `export`; local HTML Signal Deck path. |
...
| Field | Type | Description |
|--------|------|-------------|
| `subcommand` | string | `status`, `open`, `tray`, `run`, `queue`, or `history`. |
...
| `result` | object | Present for `run`/`queue`; control action result. |
