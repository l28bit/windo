...
The default GitHub branch for raw URLs is **`Genesis`**.

---

## What's new in v5.0.0 Special Edition
...
- **Genesis source cutover** — install/update, extras, checksum, and trust URLs now target the corrected **`Genesis`** branch.
...
Full list: [`CHANGELOG.md`](CHANGELOG.md). Release copy: [`docs/releases/RELEASE_NOTES_v5.0.0.md`](docs/releases/RELEASE_NOTES_v5.0.0.md).
...
| `windo surface [status\|prime\|pulse\|doctor\|repair\|open]` | **v4.2.0+ / v4.6.0+** Native surface readiness wiring, diagnostics, repair, and tray open path. |
| `windo control [status\|prime\|actions\|queue\|run\|execute-next\|inspect\|cancel\|history\|pulse\|clear]` | **v4.3.0+ / v4.4.0+** Local Windows control plane: manifest, curated action catalog, explicit JSON request queue, lifecycle states, result files, and visible-shell executor. |
| `windo signal [status\|timeline\|last\|export]` | **v4.5.0+** Signal Deck diagnostics across control requests, last elevation metadata, trust, audit chain, and native-surface readiness. |
| `windo center [status\|open\|tray\|run\|queue\|history]` | **v5.0.0+** PowerShell-native Command Center unifying tray, control, surface, motion, signal, trust, recipes, modules, extras, audit, and export. |
...
| `windo install-latest [--force] [--non-interactive] [--timeout <seconds|ms>] [--preserve-env [ALL\|name1,name2]]` | **v3.1.0+** Download and run the latest `windo_install.ps1` from **`Genesis`**. **v3.1.1+:** download only in a **non-elevated** shell; **confirm** after verify, then run installer ( **`--force`** / env for CI). |
...
**Shortcut:** **`windo repair`** (same as **`windo repair all`**) runs that safe-reset and prints reminders to **`. $PROFILE`** and run **`windo install-latest`** from a normal shell when your installed profile lags **Genesis**.
...
| `WINDO_SKIP_INSTALLER_SHA256` | Set to skip comparing downloaded `windo_install.ps1` to [`checksums/installer.sha256`](checksums/installer.sha256) on the `Genesis` branch (`bootstrap.ps1`, **`windo install-latest`** / **`upgrade`**). |
