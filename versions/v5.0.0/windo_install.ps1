<# =====================================================================
WINDO v5.0.0 Special Edition Installer
...
$WindoBuiltinVerbs = @(
    ...
    'theme', 'output', 'motion', 'surface', 'control', 'signal', 'center', 'upgrade', 'install-latest', 'uninstall', 'remove', 'profile', 'keybindings', 'completion', 'roadmap', 'syntax', 'mesh',
...
"@
    ...
    function _windo_launchpad_tray_script_text {
        $lines = @(
            ...
            '    $hint.AutoSize = $true',
            '    $hint.Location = New-Object System.Drawing.Point(24, 575)',
            ...
            '    $notify.BalloonTipText = "Ready. Right-click for preflight, surface, control plane, motion, integrity, repair, and update actions."',
            ...
            '$notify.BalloonTipText = "Native command center is running. Double-click the WINDO icon or right-click for actions, queue, signal, and repair."',
            '$notify.BalloonTipIcon = [System.Windows.Forms.ToolTipIcon]::Info',
    ...
    function _windo_control_action_catalog {
        return @(
            ...
            [pscustomobject]@{ id = "install-latest"; title = "Install Latest"; command = "windo install-latest"; group = "lifecycle"; execution = "visible-shell"; description = "Run the Genesis installer/update handoff." }
        )
    }
    ...
    function _windo_control_state {
        ...
        [pscustomobject]@{
            ...
            nextCommands = @("windo control prime", "windo control actions", "windo control queue surface-prime", "windo control execute-next", "windo center open")
            exitCode = 0
        }
    }
    ...
    function _windo_control_queue_action([string]$ActionId) {
        ...
        $payload = [ordered]@{
            ...
            note = "Queued by WINDO control plane. Execution remains explicit."
        }
...
"@
    ...
    function _windo_help_topics {
        return @(
            ...
            [pscustomobject]@{
                ...
                Syntax      = @("windo surface [status] [--json]", "windo surface prime|pulse|doctor|repair|open [--json]")
                ...
                Examples    = @("windo surface", "windo surface doctor", "windo surface repair", "windo surface open")
            },
            [pscustomobject]@{
                Name        = "control"
                Category    = "Shell Experience"
                Summary     = "Local Windows control-plane wiring for tray and visual surfaces."
                Syntax      = @("windo control [status] [--json]", "windo control prime|actions|history|pulse|clear", "windo control queue <action-id>", "windo control execute-next", "windo control inspect|cancel <request-id>", "windo control run <action-id>")
                ...
                Notes       = "Only curated WINDO action IDs can run. execute-next consumes one queued request, marks state, writes a result JSON beside the request, and leaves command output visible."
                Examples    = @("windo control", "windo control queue surface-prime", "windo control inspect <id>", "windo control execute-next", "windo control history")
            },
            [pscustomobject]@{
                Name        = "signal"
                Category    = "Inspection"
                Summary     = "Evidence-first Signal Deck for control, audit, trust, and surface state."
                Syntax      = @("windo signal [status] [--json]", "windo signal timeline [--json]", "windo signal last [--json]", "windo signal export [--open] [--output path]")
                ...
                Examples    = @("windo signal", "windo signal timeline", "windo signal last", "windo signal export --open")
            },
            [pscustomobject]@{
                Name        = "center"
                Category    = "Shell Experience"
                Summary     = "PowerShell-native WINDO Command Center."
                Syntax      = @("windo center [status] [--json]", "windo center open|tray", "windo center run <action-id>", "windo center queue <action-id>", "windo center history")
                Description = "Unifies tray, control plane, Signal Deck, native surface, motion, trust, recipes, modules, extras, audit, and export into a native-feeling Windows command center."
                ...
                Examples    = @("windo center", "windo center open", "windo center queue surface-prime", "windo center run launchpad-tray", "windo center history")
            },
            [pscustomobject]@{
                Name        = "backups"
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "surface") {
        ...
        if ($sub -notin @("status", "")) {
            if ($JsonOutput) { _emit_json "surface" @{ error = "expected status | prime | pulse | doctor | repair | open"; exitCode = 2 } }
            else { Write-Host "[windo] surface: expected status | prime | pulse | doctor | repair | open" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        $state = _windo_surface_state
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "control") {
        ...
        if ($sub -eq "queue") {
            ...
            $result = _windo_control_queue_action ([string]$Command[2])
            $exit = if ($result.ok) { 0 } else { 2 }
        ...
        if ($sub -notin @("status", "")) {
            if ($JsonOutput) { _emit_json "control" @{ error = "expected status | prime | actions | queue | run | execute-next | inspect | cancel | history | pulse | clear"; exitCode = 2 } }
            else { Write-Host "[windo] control: expected status | prime | actions | queue | run | execute-next | inspect | cancel | history | pulse | clear" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        $state = _windo_control_state
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "signal") {
        ...
        if ($sub -eq "export" -or $Command -contains "--html" -or $Command -contains "--open") {
            $signal = _windo_signal_state 80
            $htmlPath = _windo_write_signal_html $signal $outPath $openHtml
            if ($JsonOutput) { _emit_json "signal" @{ subcommand = "export"; signal = $signal; htmlPath = $htmlPath; exitCode = 0 } }
            else { Write-Host "[windo] signal deck: $htmlPath" -ForegroundColor Green }
            _windo_set_exit 0
            return
        }
        if ($sub -notin @("status", "")) {
            if ($JsonOutput) { _emit_json "signal" @{ error = "expected status | timeline | last | export"; exitCode = 2 } }
            else { Write-Host "[windo] signal: expected status | timeline | last | export" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        $signal = _windo_signal_state 25
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "center") {
        ...
        if ($sub -notin @("status", "")) {
            if ($JsonOutput) { _emit_json "center" @{ error = "expected status | open | tray | run | queue | history"; exitCode = 2 } } else { Write-Host "[windo] center: expected status | open | tray | run | queue | history" -ForegroundColor Yellow }
            _windo_set_exit 2
            return
        }
        $center = _windo_center_state
...
function __windo_completion_specs {
    @{
        help = @('version','install-latest','source','trust','scan','vault','sshx','crypto','explain','syntax','mesh','completion','output','motion','surface','control','signal','center','keybindings','recipes','venv','pkg','launchpad','preflight','dashboard','integrity','verify','config','profile','modules','extras','ai','repair')
        ...
        surface = @('status','prime','pulse','demo','doctor','repair','open','--json')
        control = @('status','prime','actions','queue','run','execute-next','inspect','cancel','history','pulse','demo','clear','surface-status','surface-prime','launchpad-tray','workbench-html','motion-pulse','profile-doctor','trust-online','preflight','install-latest','--json')
        signal = @('status','timeline','last','export','--html','--open','--output','--json')
        center = @('status','open','tray','run','queue','history','surface-status','surface-prime','launchpad-tray','workbench-html','motion-pulse','profile-doctor','trust-online','preflight','install-latest','--json')
