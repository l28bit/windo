...
"@
    ...
    function _windo_new_command_plan([object[]]$Parts) {
        ...
        { $_ -in @("version", "config", "context", "profile", "roadmap", "preflight", "dashboard", "integrity", "verify", "session", "doctor", "log", "history", "stats", "backups", "modules", "extras", "prompt", "ai", "help", "motion", "surface", "control", "signal", "center", "health", "check", "status") } {
...
function _windo_draw_ascii_startup_frame {
    ...
    function _windo_operator_actions {
        return @(
            ...
            [pscustomobject]@{ title = "Launchpad"; command = "windo launchpad --open"; note = "Open the V8.4 command center." },
    ...
    function _windo_edition_state {
        ...
        [pscustomobject]@{
            windoVersion = $WindoVersion
            edition = "WINDO V8.4 Command Center"
            generatedAt = (Get-Date).ToString("o")
            branch = _windo_release_branch
            assets = $assets
            center = $center
    ...
    function _windo_write_edition_html([object]$Edition, [string]$OutputPath, [bool]$Open) {
        ...
        $null = $sb.AppendLine("<!doctype html><html><head><meta charset='utf-8'><title>WINDO V8.4 Command Center</title>")
