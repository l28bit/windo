...
- **Ollama / local inference (Track A)** — [`docs/ai-bridge.md`](docs/ai-bridge.md) adds an **Ollama on Windows** section; **`windo ai`** tracks **`OLLAMA_HOST`** / related names and **`doctor`** can warn on non-loopback exposure; optional **`windo recipes run ollama-list`** / **`ollama-ps`** / **`ollama-version`** read-only peeks.
