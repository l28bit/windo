...
"@
    ...
    function _windo_roadmap_releases {
        return @(
            ...
            [pscustomobject]@{
                version = "8.5.3"
                codename = "Bootstrap Reliability Contract"
                theme = "Make bootstrap and update download helpers deterministic under strict mode."
                focus = @("safe missing env reads", "empty OutFile suppression", "direct raw fallback download", "managed profile block v2", "profile write backup")
                status = "in-progress"
                operatorValue = "Operators can update WINDO without strict-mode helper crashes or checksum/download helper drift."
            }
        )
    }
