...
"@
    ...
    function _windo_roadmap_releases {
        return @(
            ...
            [pscustomobject]@{
                version = "8.5.4"
                codename = "Generated Profile Prelude"
                theme = "Guarantee generated command helpers exist before argument parsing can use them."
                focus = @("profile helper prelude", "exit-code helper ordering", "timeout parser ordering", "managed profile runtime smoke", "bootstrap reliability carry-forward")
                status = "in-progress"
                operatorValue = "Operators can run update, doctor, integrity, and help paths without helper-order runtime faults in the installed profile."
            }
        )
    }
    ...
    function _windo_new_command_plan([object[]]$Parts) {
        ...
        { $_ -in @("version", "config", "context", "profile", "roadmap", "preflight", "dashboard", "integrity", "verify", "session", "doctor", "log", "history", "stats", "backups", "modules", "extras", "prompt", "ai", "help", "motion", "surface", "control", "signal", "center", "health", "check", "status", "contract", "source") } {
