<# =====================================================================
WINDO v3.6.8 Special Edition Installer
...
$WindoBuiltinVerbs = @(
    ...
    'theme', 'upgrade', 'install-latest', 'uninstall', 'remove', 'profile', 'keybindings', 'completion', 'roadmap', 'syntax', 'mesh',
    ...
    function _windo_mesh_inventory {
        ...
        return [pscustomobject]@{
            ...
            launchpad = [ordered]@{
                ...
                traySupported = [bool]$isWindowsDesktop
...
"@
    ...
    function _windo_roadmap_releases {
        return @(
            ...
            [pscustomobject]@{
                ...
                focus = @("windo mesh", "module lifecycle", "verified extras", "launchpad workflows", "structured handoff bundles")
                status = "planned"
    ...
    function _windo_run_genisis_installer {
        ...
        try {
            Write-Host "[windo] Downloading latest installer from Genisis (GitHub API first, raw fallback)..." -ForegroundColor Cyan
    ...
    function _windo_installer_raw_url {
        return "https://raw.githubusercontent.com/l28bit/windo/Genisis/windo_install.ps1"
    }

    function _windo_installer_api_url {
        return "https://api.github.com/repos/l28bit/windo/contents/windo_install.ps1?ref=Genisis"
    }
    ...
    function _windo_help_topics {
        return @(
            ...
            [pscustomobject]@{
                Name        = "mesh"
                Category    = "Planning"
                Summary     = "Preview the V4 Operator Mesh surface without executing workflows."
                Syntax      = @("windo mesh [--json]", "windo mesh doctor [--json]", "windo mesh --html [--output path|--output=path]", "windo mesh --open")
                Description = "Read-only inventory, readiness doctor, and optional local cockpit HTML that join modules, recipes, extras, launchpad/tray assets, audit state, and export readiness into one platform view."
                ...
                Examples    = @("windo mesh", "windo mesh doctor", "windo mesh --json", "windo mesh --html", "windo mesh --open")
            },
            [pscustomobject]@{
                Name        = "prompt"
                Category    = "Shell Experience"
    ...
    if ($Command.Count -ge 1 -and $Command[0] -eq "roadmap") {
        ...
        if ($JsonOutput) {
            ...
            Write-Host "  Target  : V4 platform preparation; future major reserved" -ForegroundColor Yellow
...
function __windo_completion_specs {
    @{
        help = @('version','install-latest','source','trust','explain','syntax','mesh','completion','keybindings','recipes','launchpad','preflight','dashboard','integrity','verify','config','profile','modules','extras','ai','repair')
        ...
        mesh = @('doctor','--json','--html','--open','--output')
        keybindings = @('status','doctor','set','disable','enable','reset','safe-reset','--json','--chord')
        recipes = @('list','show','preview','run','--json','--dry-run','firewall-profiles','os-version','whoami-all','ollama-list','ollama-ps','ollama-version')
